...
def test_max_pool1d_return_indices(device):
    inputs = [
        torch.randn(3, 17, device=device, requires_grad=True),
        torch.randn(3, 29, device=device, requires_grad=True),
    ]

    _compare_pool(inputs, F.max_pool1d, kernel_size=2, stride=2, return_indices=True)
