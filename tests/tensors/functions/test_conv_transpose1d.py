...
def _compare_conv_transpose1d(
    ...
    *,
    ...
    inputs = [
        torch.randn(3, 17, device=device, dtype=dtype, requires_grad=True),
...
def test_pointwise_conv_transpose1d(device):
    _compare_conv_transpose1d(device, torch.float32, kernel_size=1)


def test_spatial_conv_transpose1d(device, float_dtype):
    _compare_conv_transpose1d(
        device,
        float_dtype,
        kernel_size=3,
        stride=2,
...
def test_spatial_conv_transpose1d_output_padding_extends_tail(device):
    _compare_conv_transpose1d(
        device,
        torch.float32,
        kernel_size=3,
