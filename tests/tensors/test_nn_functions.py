...
class TestConcatTensors:
    ...
    def test__concat_tensors_dense_input_uses_canonical_dense_to_packed_path(self, device, float_dtype):
        nt = NT(
            [
                torch.arange(2 * 3, device=device, dtype=float_dtype).reshape(2, 3),
                torch.arange(3 * 3, device=device, dtype=float_dtype).reshape(3, 3),
            ]
        )
        nt._packed_sizes = None
        nt._element_shapes = None

        packed_nt, packed_dense = _concat_tensors(nt, nt.tensor)

        assert_close(packed_nt, nt.concat)
        assert_close(packed_dense, nt.concat)
...
class TestConv:
    ...
    def test_conv1d_ragged_nonzero_padding_value_matches_per_element(self, device, float_dtype):
        input = NT(
            [
    ...
    def test_conv1d_batch_first_false_dense_matches_per_element(self, device, float_dtype):
        input = NT(
            [
                torch.randn(2, 5, device=device, dtype=float_dtype),
                torch.randn(2, 5, device=device, dtype=float_dtype),
...
class TestNormalizeFunction:
    ...
    def test_normalize_ragged_axis_is_per_element(self, device, float_dtype):
        input = NT(
            [
...
class TestSoftmaxFamily:
    ...
    def test_softmax_family_ragged_axis_is_per_element(self, op, device, float_dtype):
        nt = NT(
            [
    ...
    def test_gumbel_softmax(self, device, float_dtype):
        ...
        reference = _packed_result(nt, F.gumbel_softmax(nt._values, dim=-1))
        torch.manual_seed(1016)
...
class TestDropout:
    ...
    def test_dropout_training_matches_packed_values(self, device, float_dtype):
        nt = NT(
            [
        ...
        torch.manual_seed(0)
        reference = _packed_result(nt, F.dropout(nt._values, p=0.5, training=True))
        assert_close(output, reference)
...
class TestMultiHeadAttentionForward:
    ...
    def test_multi_head_attention_forward_compile_smoke(self):
        torch.manual_seed(1017)
        embed_dim = 8
        num_heads = 2
        mha = nn.MultiheadAttention(embed_dim, num_heads, batch_first=True, dropout=0.0)
        query = NT([torch.randn(5, embed_dim), torch.randn(3, embed_dim)])

        compiled = _compile_fullgraph(
            lambda q: F.multi_head_attention_forward(
                q,
                q,
                q,
                embed_dim,
...
class TestScaledDotProductAttention:
    @pytest.mark.skipif(not hasattr(torch, "compile"), reason="torch.compile not available")
    def test_sdpa_compile_smoke(self):
        torch.manual_seed(1018)
        query = NT(
            [
                torch.randn(4, 7, 8, dtype=torch.float32),
                torch.randn(4, 3, 8, dtype=torch.float32),
            ]
        )
        compiled = _compile_fullgraph(lambda q: F.scaled_dot_product_attention(q, q, q, dropout_p=0.0))
        output = compiled(query)
    ...
    def test_sdpa_compile_cuda_float32_avoids_padded_fallback(self, device, monkeypatch):
        if device.type != "cuda":
            pytest.skip("compiled native SDPA broadening is CUDA-only")
        ...
        def _unexpected_padded_fallback(*_args, **_kwargs):
            raise RuntimeError("unexpected padded fallback")

        monkeypatch.setattr("danling.tensors.nn_functions._nested_to_padded_tensor", _unexpected_padded_fallback)

        compiled = _compile_fullgraph(lambda q, k, v: F.scaled_dot_product_attention(q, k, v, dropout_p=0.0))
        try:
    ...
    def test_attention_storage_uses_native_seq_packing(self, device, float_dtype):
    ...
    def test_create_flex_block_mask_without_packed_sizes_uses_offsets(self, monkeypatch):
        query = NT(
            [
                torch.randn(4, 11, 16, dtype=torch.float32),
                torch.randn(4, 7, 16, dtype=torch.float32),
            ]
        )
        key = NT(
            [
        ...
        query._packed_sizes = None
        key._packed_sizes = None

        captured = {}

        def _fake_create_block_mask(mask_mod, batch, num_heads, q_total, k_total, device, BLOCK_SIZE, _compile):
            captured["batch"] = batch
            captured["num_heads"] = num_heads
            captured["q_total"] = q_total
            captured["k_total"] = k_total
            captured["device"] = device
            captured["block_size"] = BLOCK_SIZE
            captured["compile"] = _compile
            return "sentinel"

        monkeypatch.setattr("danling.tensors.nn_functions._torch_create_block_mask", _fake_create_block_mask)

        output = create_flex_block_mask(lambda b, h, q_idx, kv_idx: q_idx >= kv_idx, query, key)

        assert output == "sentinel"
        assert captured["batch"] == 1
        assert captured["num_heads"] is None
        assert captured["q_total"] == 18
        assert captured["k_total"] == 18
        assert captured["device"] == query.device
    ...
    def test_sdpa_restore_native_without_python_meta(self, device, float_dtype):
        query = NT(
            [
                torch.randn(4, 11, 32, device=device, dtype=float_dtype),
                torch.randn(4, 7, 32, device=device, dtype=float_dtype),
        ...
        assert restored._packed_sizes == (11, 7)
        assert restored._element_shapes == ((4, 11, 16), (4, 7, 16))
    ...
    def test_restore_flex_dense_tensor_without_python_meta(self, device, float_dtype):
        query = NT(
            [
                torch.randn(4, 11, 32, device=device, dtype=float_dtype),
                torch.randn(4, 7, 32, device=device, dtype=float_dtype),
    ...
    def test_packed_sizes_tuple_uses_offsets_without_python_meta(self, device, float_dtype):
        attention_like = NT(
            [
                torch.randn(4, 11, 32, device=device, dtype=float_dtype),
                torch.randn(4, 7, 32, device=device, dtype=float_dtype),
        ...
        assert _packed_sizes_tuple(attention_like) == (11, 7)
        assert _packed_sizes_tuple(conv_like) == (35, 24)
    ...
    @pytest.mark.skipif(create_block_mask is None or flex_attention is None, reason="FlexAttention not available")
    def test_flex_attention_wrapper_matches_per_element_sdpa(self, device):
        if device.type != "cuda":
    ...
    @pytest.mark.skipif(create_block_mask is None or flex_attention is None, reason="FlexAttention not available")
    def test_flex_attention_wrapper_does_not_depend_on_torch_nested(self, device, monkeypatch):
        if device.type != "cuda":
            pytest.skip("DanLing FlexAttention wrapper is currently CUDA-focused")

        if not hasattr(torch.nested, "nested_tensor_from_jagged"):
            pytest.skip("torch.nested.nested_tensor_from_jagged unavailable")

        query = NT(
            [
                torch.randn(4, 17, 16, device=device, dtype=torch.float32),
                torch.randn(4, 9, 16, device=device, dtype=torch.float32),
            ]
        )
        key = NT(
            [
        ...
        monkeypatch.setattr(
            ...
            lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("unexpected torch.nested dependency")),
        )
...
class TestPairwiseDistance:
    ...
    def test_pairwise_distance_without_python_meta(self, device, float_dtype):
        x1 = nested_rand([(2, 3), (1, 3)], device, float_dtype)
        x2 = nested_rand([(2, 3), (1, 3)], device, float_dtype)
        x1._packed_sizes = None
        x1._element_shapes = None
        x2._packed_sizes = None
        x2._element_shapes = None

        output = F.pairwise_distance(x1, x2, keepdim=True)
        reference = NT([F.pairwise_distance(a, b, keepdim=True) for a, b in zip(x1, x2)], **x1._meta())
        assert_close(output, reference, atol=1e-6, rtol=1e-6)
...
class TestOneHot:
    ...
    def test_one_hot_without_python_meta(self):
        x = NT([torch.tensor([0, 1, 2], dtype=torch.long), torch.tensor([1, 0], dtype=torch.long)])
        x._packed_sizes = None
        x._element_shapes = None
...
class TestFractionalMaxPool:
    ...
    def test_fractional_max_pool2d_with_return_indices(self, device, float_dtype):
        x = NT(
            [
                torch.randn(1, 4, 4, device=device, dtype=float_dtype),
                torch.randn(1, 4, 4, device=device, dtype=float_dtype),
        ...
        random_samples = torch.tensor([[[0.5, 0.5]]], dtype=float_dtype, device=device)
        output, idx = F.fractional_max_pool2d(
            x, kernel_size=2, output_size=2, _random_samples=random_samples, return_indices=True
        )
        assert isinstance(output, NestedTensor)
        assert isinstance(idx, NestedTensor)
    ...
    def test_fractional_max_pool3d_with_return_indices(self, device, float_dtype):
        x = NT(
            [
                torch.randn(1, 4, 4, 4, device=device, dtype=float_dtype),
                torch.randn(1, 4, 4, 4, device=device, dtype=float_dtype),
        ...
        random_samples = torch.tensor([[[0.3, 0.3, 0.3]]], dtype=float_dtype, device=device)
        output, idx = F.fractional_max_pool3d(
            x, kernel_size=2, output_size=2, _random_samples=random_samples, return_indices=True
        )
        assert isinstance(output, NestedTensor)
        assert isinstance(idx, NestedTensor)
