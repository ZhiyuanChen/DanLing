...
def test_from_tensor_mask_batch_mismatch_raises():
...
def test_torch_function_scalar_batch_fallback():
...
def test_from_concatenated_extra_elements_raises():
...
def test_from_concatenated_same_shapes():
...
@pytest.mark.skipif(TORCH_VERSION < Version("1.12"), reason="requires PyTorch 1.12 or higher")
@pytest.mark.parametrize(
    "i",
    [
        NestedTensor([[6, 5, 4], [3, 2]]),
...
def test_tensor_mask_shapes_for_1d_sequences():
    lengths = [2, 3, 5, 7]
    channels = 8
...
def test_cat_extends_1d_sequence_max_length():
    lengths = [2, 3, 5, 7]
...
def test_view_insert_dim_before_dynamic_length():
...
def test_cuda_move_keeps_state():
    nested_tensor = NestedTensor(
...
def test_getitem_nested_index_length_mismatch():
    nested_tensor = NestedTensor([torch.tensor([1, 2]), torch.tensor([3])])
...
def test_sum_multi_dim_batch_first_false():
    nested_tensor = NestedTensor(
