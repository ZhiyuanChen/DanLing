...
class TestShapeManipulation:
    ...
    def test_view_compile_smoke(self):
        nested_tensor = NestedTensor([torch.randn(2, 3), torch.randn(4, 3)])
        compiled = torch.compile(lambda x: x.view(-1, 3), backend="eager", fullgraph=True)
        output = compiled(nested_tensor)
        reference = nested_tensor.view(-1, 3)
        assert_close(output, reference)

    @pytest.mark.skipif(not hasattr(torch, "compile"), reason="torch.compile not available")
    def test_nested_like_compile_smoke(self):
        nested_tensor = NestedTensor([torch.randn(2, 3), torch.randn(4, 3)])
        dense = nested_tensor.tensor
        compiled = torch.compile(lambda x, y: x.nested_like(y), backend="eager", fullgraph=True)
        output = compiled(nested_tensor, dense)
        reference = nested_tensor.nested_like(dense)
        assert_close(output, reference)

    @pytest.mark.skipif(not hasattr(torch, "compile"), reason="torch.compile not available")
    def test_nested_like_larger_dense_compile_smoke(self):
        nested_tensor = NestedTensor([torch.randn(2, 3), torch.randn(4, 3)])
        dense = torch.randn(2, 6, 3)
        compiled = torch.compile(lambda x, y: x.nested_like(y, strict=False), backend="eager", fullgraph=True)
        output = compiled(nested_tensor, dense)
    ...
    @pytest.mark.skipif(not hasattr(torch, "compile"), reason="torch.compile not available")
    def test_reshape_compile_smoke(self):
        nested_tensor = NestedTensor([torch.randn(2, 3), torch.randn(4, 3)])
        compiled = torch.compile(lambda x: x.reshape(-1, 3), backend="eager", fullgraph=True)
        output = compiled(nested_tensor)
