...
def assert_nested_close(output, reference, *, atol=1e-6, rtol=1e-6):
    """Wrapper over torch.testing.assert_close that understands NestedTensor."""

    if not isinstance(output, NestedTensor):
        return assert_close(output, reference, atol=atol, rtol=rtol)

    if not isinstance(reference, NestedTensor):
        reference = output.nested_like(reference)

    for out, reference_elem in zip(output._storage, reference._storage):
        assert_close(out, reference_elem, atol=atol, rtol=rtol)
