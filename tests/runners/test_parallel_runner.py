...
class TestParallelRunnerRuntimeBehavior:
    ...
    def test_pipeline_loss_uses_token_weighted_sum(self) -> None:
        ...
        try:
            runner.criterion = lambda pred, target: nn.functional.mse_loss(pred, target["value"])
            runner._pipeline_loss_mode = "train"
            runner._pipeline_loss_normalizer_local = 0.0
            runner._accumulation_normalizer_local = 0.0

            loss_a = runner._pipeline_loss(
                torch.ones(1, 1),
                {"value": torch.zeros(1, 1), "tokens": NestedTensor([torch.zeros(1)])},
            )
            loss_b = runner._pipeline_loss(
                torch.full((3, 1), 3.0),
                {"value": torch.zeros(3, 1), "tokens": NestedTensor([torch.zeros(3)])},
            )
            weighted = runner._pipeline_loss_value([loss_a, loss_b])

            assert weighted is not None
            assert weighted.item() == pytest.approx(7.0)
            assert runner._pipeline_loss_normalizer_local == pytest.approx(4.0)
            assert runner._accumulation_normalizer_local == pytest.approx(4.0)
        finally:
            runner._pipeline_loss_mode = None
            runner.close()

    def test_pipeline_loss_normalizer_ignores_top_level_batch(self) -> None:
        ...
        try:
            assert runner.loss_normalizer({"target": NestedTensor([torch.ones(4)])}) is None
        finally:
            runner.close()
