...
class TestRegressionMetrics:
    def test_metrics(self, device) -> None:
    ...
    def test_correlation_metrics_support_multioutput_reductions(self, device) -> None:
        ...
        target_weights = targets.var(dim=0, correction=0)
        weighted_pearson = (pearson_raw * target_weights).sum() / target_weights.sum()
        weighted_spearman = (spearman_raw * target_weights).sum() / target_weights.sum()
        ...
        assert_metric_close(pearson(multioutput="variance_weighted")(state), weighted_pearson)
        ...
        assert_metric_close(spearman(multioutput="variance_weighted")(state), weighted_spearman)

    def test_multioutput_rejects_invalid_mode(self, device) -> None:
        state = MetricState(preds=PREDS.to(device), targets=TARGETS.to(device))
    ...
    def test_return_nan_on_empty_state(self, metric, device) -> None:
        empty = MetricState(preds=torch.tensor([], device=device), targets=torch.tensor([], device=device))
        value = torch.as_tensor(metric(empty))
