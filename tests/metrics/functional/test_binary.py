...
from danling.metrics.functional import binary_f1_score, binary_fbeta_score
...
class TestBinaryMetrics:
    ...
    @pytest.mark.parametrize(
        "metric,expected_fn",
        [
            (binary_accuracy(), lambda p, t: tmfc.binary_accuracy(p, t, threshold=0.5, ignore_index=-100)),
            (binary_precision(), lambda p, t: tmfc.binary_precision(p, t, threshold=0.5, ignore_index=-100)),
            (binary_recall(), lambda p, t: tmfc.binary_recall(p, t, threshold=0.5, ignore_index=-100)),
        ],
    )
    def test_threshold_boundary(self, metric, expected_fn: Callable, device) -> None:
        preds = torch.tensor([0.50, 0.49, 0.51, 0.50], device=device)
    ...
    def test_return_nan_on_zero_confmat_totals(self, device) -> None:
        confmat = torch.zeros((2, 2), device=device)
        state = MetricState(
            preds=torch.tensor([], device=device),
            targets=torch.tensor([], device=device),
            confmat=confmat,
    ...
    def test_f1_alias(self) -> None:
        ...
        assert binary_f1_score is binary_f1

    def test_fbeta_alias(self) -> None:
        assert binary_fbeta_score is binary_fbeta
