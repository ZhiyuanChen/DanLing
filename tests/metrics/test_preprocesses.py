...
def test_preprocess():
    ...
    # Ignore NaN
    input_tensor = torch.tensor([0.1, 0.5, float("nan"), 0.3])
    ...
    assert torch.allclose(processed_input, torch.tensor([0.1, 0.5]), equal_nan=False)
    assert torch.allclose(processed_target, torch.tensor([0.0, 1.0]), equal_nan=False)
...
def test_task_specific_preprocess():
    ...
    # Regression with NaN
    input_tensor = torch.tensor([0.1, float("nan"), 0.3])
    ...
    assert processed_input.shape == torch.Size([1])
    assert processed_target.shape == torch.Size([1])
    assert torch.allclose(processed_input, torch.tensor([0.1]), equal_nan=False)
    assert torch.allclose(processed_target, torch.tensor([0.1]), equal_nan=False)
