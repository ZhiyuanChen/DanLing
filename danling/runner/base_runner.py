...
class BaseRunner(metaclass=RunnerMeta):  # pylint: disable=too-many-public-methods
    ...
    def deepspeed_config(  # pylint: disable=too-many-branches, too-many-configments
        ...
        if config is None:
            config = self.config.get("deepspeed")
        if config is None:
            return {}
    ...
    def save_checkpoint(self) -> None:
        ...
        If `self.config.save_interval` is positive and `self.config.epochs + 1` is a multiple of `save_interval`,
        the checkpoint will also be copied to `self.checkpoint_dir/epoch-{self.config.epochs}.pth`.
        ...
        if (
            ...
            and (self.config.epochs + 1) % self.save_interval == 0
        ):
            save_path = os.path.join(self.checkpoint_dir, f"epoch-{self.config.epochs}.pth")
    ...
    def load_checkpoint(
        ...
        **kwargs,
        ...
        self.config.iter_begin = self.config.iters
        self.config.step_begin = self.config.steps
    ...
    def scores(self) -> FlatDict | None:
        ...
        Scores are extracted from results by `score_set` and `runner.config.score_name`,
        following `[r[score_set][self.config.score_name] for r in self.results]`.
        ...
        `score_set` is defined in `self.config.score_set`.
        ...
        score_set = self.config.get("score_set")
        ...
        return FlatDict({k: v[score_set][self.config.score_name] for k, v in self.results.items()})
