...
class DeepSpeedRunner(TorchRunner):
    ...
    def save_checkpoint(self, epoch: int | None = None) -> None:
        ...
        The checkpoint will be saved to `self.checkpoint_dir/latest`.

        If `self.config.save_interval` is positive and `self.config.epochs + 1` is a multiple of `save_interval`,
        the checkpoint will also be copied to `self.checkpoint_dir/epoch-{self.config.epochs}`.

        If `self.config.save_interval` is non-negative and `self.is_best` is `True`, the checkpoint will also be
        copied to `self.checkpoint_dir/best`.
        """

        epoch = epoch or self.config.epochs
        ...
        self.model.save_checkpoint(
            self.checkpoint_dir, tag="latest", client_state={"runner": self.config.dict()}, save_latest=False
        )
        ...
        if save_interval > -1 and self.is_best:
