...
class NestedTensor(torch.Tensor):
    ...
    def _storage_cache_dropped_grad(self, cached: tuple[Tensor, ...]) -> bool:
        ...
        """
        return bool(cached) and self.concat.requires_grad and cached[0].grad_fn is None
    ...
    def concat(self) -> Tensor:
        ...
        if vars(self).get("_unflattened_autograd", False):
            if _is_fake_tensor(values):
                ...
                # AOT also inspects functional children outside their dispatch mode.
                # Only active functional execution can record the wrapper projection.
                if (
                    not isinstance(values, FunctionalTensor)
                    or _detect_infra_mode(torch._C._TorchDispatchModeKey.FUNCTIONAL) is None
                ):
                    return values
            return _project_packed_values(self) if self.requires_grad else values
        if self.grad_fn is not None and not values.requires_grad:
