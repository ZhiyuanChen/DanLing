...
This module defines the storage contract (``concat``, ``_offsets``,
...
from typing import TYPE_CHECKING, Any, Callable, ClassVar, Iterable, SupportsFloat, TypeVar, cast, overload
...
if TYPE_CHECKING:
    from torch._C import _Index

_NestedTensorT = TypeVar("_NestedTensorT", bound="NestedTensor")
_PackedConstructor = Callable[
    [
        Tensor,
        Tensor,
        Tensor,
...
class NestedTensor(torch.Tensor):
    ...
    def __new__(
        ...
        *tensors: Iterable[Tensor],
        ...
        if torch.is_grad_enabled() and values.requires_grad:
            return _PackedLikeAutograd.apply(values, _PackedStructureReference(result, reusable=True))
        return result
    ...
    def packed_local_indices(
        ...
        *,
        ...
        if (
            ...
            total = self.concat.shape[0] if is_last_level else hierarchical_offsets[normalized_level + 1].numel() - 1
    ...
    def concat(self) -> Tensor:
        ...
        if (
            ...
            and self._cache_version(self) == 0
            and getattr(grad_fn, "_forward_cls", None) is _PackedLikeAutograd
        ):
            return values
        token = (
            (id(values), self._cache_version(self), self._cache_version(values), *self._shape_cache_token())
            if cacheable
            else ()
        )
    ...
    def _packed_like_unchecked_raw(self, packed_values: Tensor) -> Self:
        ...
        if (
            ...
            result._invalidate_transient_caches()
            # These conversions depend only on the very same metadata tensors
            # and their version counters. Never inherit numerical/autograd views.
    ...
    def _packed_like_unchecked(self, packed_values: Tensor, *, reuse_wrapper: bool = False) -> Self:
        ...
        if reuse_wrapper and not _is_compiling():
            from torch.utils._python_dispatch import _get_current_dispatch_mode
...
class _PackedStructureReference:
    r"""Forward-only carrier for layout metadata used by the packed autograd bridge."""

    __slots__ = ("value", "reuse_wrapper", "reusable")

    def __init__(self, value: NestedTensor, *, reuse_wrapper: bool = False, reusable: bool = False) -> None:
        self.value = value
        self.reuse_wrapper = reuse_wrapper
...
class _PackedLikeAutograd(torch.autograd.Function):
    ...
    def forward(ctx, packed_values: Tensor, structure: _PackedStructureReference) -> NestedTensor:
        ...
        # A full wrapper also owns numerical storage. Saving a detached layout
        # through autograd lets checkpoint hooks release/recompute that storage;
        # retaining the wrapper in an opaque ctx attribute would keep it alive.
        layout = reference.detach()
        max_length_binding = vars(reference).get("_compile_max_length_binding")
        if max_length_binding is not None:
            layout._max_length_binding = max_length_binding
        if (
            structure.reusable
            and not _is_compiling()
            and not _is_fake_tensor(packed_values)
            and not reference._pin_memory
        ):
            # Eagerly factory-built wrappers are reused as graph inputs across independent
            # forward/backward steps (AOTAutograd attaches CompiledFunctionBackward to the
            # wrapper edge), so this node must survive repeated backward passes. Keep only
            # layout metadata: a zero-stride one-element child preserves the packed row
            # count without retaining activation storage, so checkpoint regions can still
            # release the values this wrapper was built from. Pinned layouts keep the
            # saved-tensor path because re-pinning would materialise the expanded child.
            ctx.reusable_layout = layout._packed_like_unchecked_raw(
                layout._packed_values.new_empty(()).expand(ctx.packed_shape)
            )
        else:
            ctx.save_for_backward(layout)
        result = reference if structure.reuse_wrapper else reference._packed_like_unchecked_raw(packed_values)
        if max_length_binding is not None:
            result._max_length_binding = max_length_binding
        return result
    ...
    def backward(ctx, grad_output: NestedTensor):
        reference = getattr(ctx, "reusable_layout", None)
        if reference is None:
            (reference,) = ctx.saved_tensors
        values_grad = grad_output.concat if isinstance(grad_output, NestedTensor) else grad_output
...
class _PackedValuesAutograd(torch.autograd.Function):
    ...
    def forward(ctx, input: NestedTensor) -> Tensor:
        # Only the layout is needed in backward. Its detached alias keeps version
        # checks and saved-tensor hooks without retaining the owning wrapper.
        ctx.save_for_backward(input.detach())
        # The dense child is not a Function input. Returning that exact object
        # lets autograd attach this node to the wrapper's own backing tensor,
        # creating a wrapper -> backing.grad_fn -> saved-wrapper reference cycle.
        # A fresh view preserves storage and version aliases without mutating it.
        return input._packed_values.view_as(input._packed_values)
...
def _traceable_repeat_batch(self: _NestedTensorT, repeats: int, *, output_size: int | None = None) -> _NestedTensorT:
    r"""Repeat complete logical batch elements in interleaved order."""
    from .torch_functions import _repeat_interleave_packed_batch

    return cast(_NestedTensorT, _repeat_interleave_packed_batch(self, repeats, output_size=output_size))
