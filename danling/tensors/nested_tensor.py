...
class NestedTensor:
        ...
        *tensors: Iterable[Tensor],
        ...
        if len(tensors) == 1 and not isinstance(tensors[0], Tensor):
            if isinstance(tensors[0], Iterable):
                ...
                raise ValueError(f"tensors must be an Iterable, bug got {type(tensors[0])}.")
    ...
    def _storage(self, tensors: Sequence):
        ...
        for t in tensors:
            if not isinstance(t, Tensor):
                t = torch.tensor(t, dtype=self._dtype, device=self._device, pin_memory=self._pin_memory)
            else:
                t = t.to(self._device, dtype=self._dtype)
            if self._requires_grad is not None:
                t.requires_grad_(self._requires_grad)
            tensors_.append(t)
        if len(tensors_) == 0:
            self.__storage = ()
            return
        devices = {t.device for t in tensors_}
        if len(devices) != 1:
            raise ValueError(
                "All tensors in NestedTensor must be on the same device, "
                f"but got {sorted(str(device) for device in devices)}"
            )
        common_dtype = tensors_[0].dtype
        for t in tensors_[1:]:
            common_dtype = torch.promote_types(common_dtype, t.dtype)
        if any(t.dtype != common_dtype for t in tensors_):
            tensors_ = [t.to(dtype=common_dtype) for t in tensors_]
        tensors = tuple(tensors_)
        self._dtype = tensors[0].dtype
        self._device = tensors[0].device
        self._requires_grad = tensors[0].requires_grad
    ...
    def dtype(self) -> torch.dtype | None:
        if self._storage:
            return self._storage[0].dtype
        return self._dtype if self._dtype is not None else torch.get_default_dtype()

    @dtype.setter
    def dtype(self, value: torch.dtype | None):
        self._dtype = value
        if self._storage and value is not None:
            self._storage = tuple(t.to(dtype=value) for t in self._storage)

    @property
    def device(self) -> torch.device | None:
        if self._storage:
            return self._storage[0].device
        return self._device
    ...
    @property
    def requires_grad(self) -> bool | None:
        if self._storage:
            return self._storage[0].requires_grad
    ...
    def tensor_mask(self) -> Tuple[Tensor, Tensor]:
        ...
        if not self._storage:
            size = self.size()
            tensor = torch.empty(size, dtype=self._default_dtype(), device=self.device)
            mask = torch.empty(size, dtype=torch.bool, device=self.device)
            return tensor, mask
        return self._tensor_mask(self._storage, self.batch_first, self.padding_value, self.mask_value)

    def _tensor_mask(
        self, storage: tuple, batch_first: bool, padding_value: SupportsFloat, mask_value: bool
    ) -> Tuple[Tensor, Tensor]:
        if not storage:
            size = self.size()
            tensor = torch.empty(size, dtype=self._default_dtype(), device=self.device)
    ...
    def concatenate(self) -> Tuple[Tensor, Tuple[torch.Size, ...]]:
        ...
        if elem.ndim == 0:
            concat_tensor = torch.stack(self._storage, dim=0)
        ...
        if elem.shape == torch.Size(shape) and all(t.shape == elem.shape for t in self._storage):
    ...
    def from_concatenated(cls, concat_tensor: Tensor, shapes: Tuple[torch.Size, ...], **kwargs) -> Self:
        ...
        if num_provided < total_expected:
            raise ValueError(
                f"Concatenated tensor has {num_provided} elements but "
                f"expected at least {total_expected} based on shapes {shapes}"
            )
        if num_provided > total_expected:
            raise ValueError(
                f"Concatenated tensor has {num_provided} elements but "
                f"expected exactly {total_expected} based on shapes {shapes}"
            )
    ...
    @classmethod
    def __torch_function__(cls, func, types, args=(), kwargs=None) -> Self | NestedTensor | Tensor | Tuple:
        if kwargs is None:
            kwargs = {}
    ...
    @classmethod
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None) -> Self | NestedTensor | Tensor | Tuple:
        if kwargs is None:
            kwargs = {}
        source = next((a for a in args if isinstance(a, NestedTensor)), None)
        if source is None:
    ...
    def _inherit_state_from(self, other: Self) -> Self:
        ...
        if not self._storage:
            ...
            self._requires_grad = other.requires_grad
        return self
    ...
    def _size(
        ...
        if dim is not None:
            ...
            if dim == 1:
                return len(storage)
            return max(t.size(dim if dim == 0 else dim - 1) for t in storage)
    ...
    def _view_shapes(self, shape) -> list[tuple[int, ...]]:
        ...
        if has_batch:
            if len(shape) != self.dim():
                ...
                for i, dim in enumerate(tensor_dims):
                    if i < len(max_sizes) and (shape[dim] == -1 or shape[dim] == max_sizes[i]):
                        include_batch = True
                        break
