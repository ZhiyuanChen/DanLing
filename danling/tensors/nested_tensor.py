...
class _PackedStructureReference:
    ...
    ``mark_dynamic`` flags a public factory result, which keeps Dynamo's dynamic-dim hints
    (see :meth:`NestedTensor._mark_tensor_backed_dynamic_dims`).
    """

    __slots__ = ("value", "reuse_wrapper", "mark_dynamic")

    def __init__(self, value: NestedTensor, *, reuse_wrapper: bool = False, mark_dynamic: bool = False) -> None:
        self.value = value
...
class _PackedLikeAutograd(torch.autograd.Function):
    ...
    def forward(ctx, packed_values: Tensor, structure: _PackedStructureReference) -> NestedTensor:
        ...
        if not _is_compiling() and not _is_fake_tensor(packed_values):
            # Backward needs layout metadata only: the axis split for a packed tangent and, for
            # a dense tangent, a storage-free layout wrapper rebuilt from these metadata tensors.
            # Nothing here retains activation storage, so checkpoint regions can release the
            # values this wrapper was built from, and the node survives the repeated backward
            # passes of a factory-built wrapper reused as a graph input (AOTAutograd attaches
            # CompiledFunctionBackward to the wrapper edge).
            ctx.layout_state = reference._layout_state()
            ctx.axes = (reference._ragged_dims, reference._static_dims)
        else:
            # A full wrapper also owns numerical storage. Saving a detached layout through
            # autograd lets checkpoint hooks release/recompute that storage; retaining the
            # wrapper in an opaque ctx attribute would keep it alive.
        ...
        if max_length_binding is not None:
            result._max_length_binding = max_length_binding
        if structure.mark_dynamic:
            result._mark_tensor_backed_dynamic_dims()
        return result

    @staticmethod
    def backward(ctx, grad_output: NestedTensor):
        state = getattr(ctx, "layout_state", None)
        if state is None:
            (reference,) = ctx.saved_tensors
            reference_ragged, reference_static = reference._ragged_dims, reference._static_dims
        else:
            reference = None
            reference_ragged, reference_static = ctx.axes
        ...
        if isinstance(grad_output, NestedTensor):
            ...
            packed_tangent = grad_output._ragged_dims == reference_ragged and set(grad_static) == set(reference_static)
        else:
            grad_static = ()
        ...
        if packed_tangent:
            ...
            if reference is None:
                reference = NestedTensor._layout_from_state(ctx.layout_state, values_grad, ctx.packed_shape)
