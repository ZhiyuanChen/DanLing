...
- `_shape_tensor`: Per-element shapes, shape `(B, ndim)`, recording each element's original dimensions
...
- `_map_storage(input, fn)`: Per-element dispatch — applies `fn` to each element via `_unpack()`. Used when ops need individual element dimensionality.
...
from danling.tensors.ops import _map_storage

@NestedTensorFuncRegistry.implement(torch.my_custom_op)
def my_custom_op(input, *args, **kwargs):
    # For per-element ops, use _map_storage:
    return _map_storage(input, lambda t: torch.my_custom_op(t, *args, **kwargs))
```
...
def _my_handler(func, args, kwargs):
    ...
    return type(source)._from_packed(
        ...
        source._offsets, source._shape_tensor,
