...
def abs(tensor: NestedTensor) -> NestedTensor:
    r"""
    Computes the absolute value of each element in input. See also [torch.abs][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([-3.0]))
        >>> torch.equal(torch.abs(nt), torch.abs(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.abs)


@NestedTensorFuncRegistry.implement(torch.neg)
def neg(input: NestedTensor) -> NestedTensor:
    r"""
    ...
    Examples:
        ...
        >>> torch.allclose(torch.neg(nt), torch.neg(nt.tensor))
        True
    """
    return _concat_apply_same_shape(input, torch.neg)


@NestedTensorFuncRegistry.implement(torch.positive)
def positive(input: NestedTensor) -> NestedTensor:
    r"""
    Returns :attr:`input`.
    See also [torch.positive][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.positive(nt), torch.positive(nt.tensor))
        True
    """
    return _concat_apply_same_shape(input, torch.positive)


@NestedTensorFuncRegistry.implement(torch.bitwise_not)
def bitwise_not(input: NestedTensor) -> NestedTensor:
    r"""
    Computes the bitwise NOT of the given input tensor. The input tensor must be of integral or Boolean types. For bool
    tensors, it computes the logical NOT.
    See also [torch.bitwise_not][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.bitwise_not(nt), torch.bitwise_not(nt.tensor))
        True
    """
    return _concat_apply_same_shape(input, torch.bitwise_not)
...
def cos(tensor: NestedTensor) -> NestedTensor:
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.cos(nt), torch.cos(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.cos)
...
@NestedTensorFuncRegistry.implement(torch.true_divide)
def true_divide(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    Alias for :func:`torch.div` with ``rounding_mode=None``.
    See also [torch.true_divide][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> torch.allclose(torch.true_divide(nt, 1.5), torch.true_divide(nt.tensor, 1.5))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.true_divide)


@NestedTensorFuncRegistry.implement(torch.floor_divide)
def floor_divide(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    .. note:: Before PyTorch 1.13 :func:`torch.floor_divide` incorrectly performed truncation division. To restore the
    previous behavior use :func:`torch.div` with ``rounding_mode='trunc'``.
    See also [torch.floor_divide][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> torch.allclose(torch.floor_divide(nt, 1.5), torch.floor_divide(nt.tensor, 1.5))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.floor_divide)


@NestedTensorFuncRegistry.implement(torch.remainder)
def remainder(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    Computes `Python's modulus operation <https://docs.python.org/3/reference/expressions.html#binary-arithmetic-
    operations>`_ entrywise.  The result has the same sign as the divisor :attr:`other` and its absolute value is less
    than that of :attr:`other`.
    See also [torch.remainder][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> torch.allclose(torch.remainder(nt, 1.5), torch.remainder(nt.tensor, 1.5))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.remainder)


@NestedTensorFuncRegistry.implement(torch.eq)
def eq(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    Computes element-wise equality The second argument can be a number or a tensor whose shape is :ref:`broadcastable
    <broadcasting-semantics>` with the first argument.
    See also [torch.eq][].
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.eq(nt, 2), torch.eq(nt.tensor, 2))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.eq)


@NestedTensorFuncRegistry.implement(torch.ne)
def ne(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.ne(nt, 2), torch.ne(nt.tensor, 2))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.ne)


@NestedTensorFuncRegistry.implement(torch.gt)
def gt(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.gt(nt, 2), torch.gt(nt.tensor, 2))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.gt)


@NestedTensorFuncRegistry.implement(torch.ge)
def ge(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.ge(nt, 2), torch.ge(nt.tensor, 2))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.ge)


@NestedTensorFuncRegistry.implement(torch.lt)
def lt(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    Computes :math:`\text{input} < \text{other}` element-wise.
    See also [torch.lt][].
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.lt(nt, 2), torch.lt(nt.tensor, 2))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.lt)


@NestedTensorFuncRegistry.implement(torch.le)
def le(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    r"""
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.le(nt, 2), torch.le(nt.tensor, 2))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.le)


@NestedTensorFuncRegistry.implement(torch.logical_and)
def logical_and(input, other):
    r"""
    Computes the element-wise logical AND of the given input tensors. Zeros are treated as ``False`` and nonzeros are
    treated as ``True``.
    See also [torch.logical_and][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([True, False]), torch.tensor([False, True]))
        >>> torch.equal(torch.logical_and(nt, torch.tensor(True)), torch.logical_and(nt.tensor, torch.tensor(True)))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.logical_and)


@NestedTensorFuncRegistry.implement(torch.logical_or)
def logical_or(input, other):
    r"""
    Computes the element-wise logical OR of the given input tensors. Zeros are treated as ``False`` and nonzeros are
    treated as ``True``.
    See also [torch.logical_or][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([True, False]), torch.tensor([False, True]))
        >>> torch.equal(torch.logical_or(nt, torch.tensor(True)), torch.logical_or(nt.tensor, torch.tensor(True)))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.logical_or)


@NestedTensorFuncRegistry.implement(torch.logical_xor)
def logical_xor(input, other):
    r"""
    Computes the element-wise logical XOR of the given input tensors. Zeros are treated as ``False`` and nonzeros are
    treated as ``True``.
    See also [torch.logical_xor][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([True, False]), torch.tensor([False, True]))
        >>> torch.equal(torch.logical_xor(nt, torch.tensor(True)), torch.logical_xor(nt.tensor, torch.tensor(True)))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.logical_xor)


@NestedTensorFuncRegistry.implement(torch.bitwise_and)
def bitwise_and(input, other):
    r"""
    Computes the bitwise AND of :attr:`input` and :attr:`other`. The input tensor must be of integral or Boolean types.
    For bool tensors, it computes the logical AND.
    See also [torch.bitwise_and][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.bitwise_and(nt, 1), torch.bitwise_and(nt.tensor, 1))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.bitwise_and)


@NestedTensorFuncRegistry.implement(torch.bitwise_or)
def bitwise_or(input, other):
    r"""
    Computes the bitwise OR of :attr:`input` and :attr:`other`. The input tensor must be of integral or Boolean types.
    For bool tensors, it computes the logical OR.
    See also [torch.bitwise_or][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.bitwise_or(nt, 1), torch.bitwise_or(nt.tensor, 1))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.bitwise_or)


@NestedTensorFuncRegistry.implement(torch.bitwise_xor)
def bitwise_xor(input, other):
    r"""
    Computes the bitwise XOR of :attr:`input` and :attr:`other`. The input tensor must be of integral or Boolean types.
    For bool tensors, it computes the logical XOR.
    See also [torch.bitwise_xor][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.bitwise_xor(nt, 1), torch.bitwise_xor(nt.tensor, 1))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.bitwise_xor)


@NestedTensorFuncRegistry.implement(torch.bitwise_left_shift)
def bitwise_left_shift(input, other):
    r"""
    Computes the left arithmetic shift of :attr:`input` by :attr:`other` bits.
    See also [torch.bitwise_left_shift][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.bitwise_left_shift(nt, 1), torch.bitwise_left_shift(nt.tensor, 1))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.bitwise_left_shift)


@NestedTensorFuncRegistry.implement(torch.bitwise_right_shift)
def bitwise_right_shift(input, other):
    r"""
    Computes the right arithmetic shift of :attr:`input` by :attr:`other` bits.
    See also [torch.bitwise_right_shift][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1, 2]), torch.tensor([3, 4]))
        >>> torch.equal(torch.bitwise_right_shift(nt, 1), torch.bitwise_right_shift(nt.tensor, 1))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.bitwise_right_shift)
...
def exp(tensor: NestedTensor) -> NestedTensor:
    r"""
    Returns a new tensor with the exponential of the elements of the input tensor :attr:`input`.
    See also [torch.exp][].
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.exp(nt), torch.exp(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.exp)
...
@NestedTensorFuncRegistry.implement(torch.isfinite)
def isfinite(input: NestedTensor) -> NestedTensor:
    r"""
    Returns a new tensor with boolean elements representing if each element is `finite` or not.
    See also [torch.isfinite][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, float('nan')]), torch.tensor([2.0, 3.0]))
        >>> torch.equal(torch.isfinite(nt), torch.isfinite(nt.tensor))
        True
    """
    return _concat_apply_same_shape(input, torch.isfinite)


@NestedTensorFuncRegistry.implement(torch.isnan)
def isnan(input: NestedTensor) -> NestedTensor:
    r"""
    Returns a new tensor with boolean elements representing if each element of :attr:`input` is NaN or not. Complex
    values are considered NaN when either their real and/or imaginary part is NaN.
    See also [torch.isnan][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, float('nan')]), torch.tensor([2.0, 3.0]))
        >>> torch.equal(torch.isnan(nt), torch.isnan(nt.tensor))
        True
    """
    return _concat_apply_same_shape(input, torch.isnan)
...
def log(tensor):
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> torch.allclose(torch.log(nt), torch.log(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.log)
...
def aminmax(input: NestedTensor, *, dim: int | None = None, keepdim: bool = False):
    ...
    try:
        ...
        return NestedTensor(mins, **input._state), NestedTensor(maxs, **input._state)


@NestedTensorFuncRegistry.implement(torch.maximum)
def maximum(input, other):
    r"""
    Computes the element-wise maximum of :attr:`input` and :attr:`other`.
    See also [torch.maximum][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> other = torch.tensor([2.0, 1.0])
        >>> torch.allclose(torch.maximum(nt, other), torch.maximum(nt.tensor, other))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.maximum)
...
@NestedTensorFuncRegistry.implement(torch.minimum)
def minimum(input, other):
    r"""
    Computes the element-wise minimum of :attr:`input` and :attr:`other`.
    See also [torch.minimum][].
    ...
    Examples:
        ...
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> other = torch.tensor([2.0, 1.0])
        >>> torch.allclose(torch.minimum(nt, other), torch.minimum(nt.tensor, other))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.minimum)
...
def mul(input: NestedTensor, other: NestedTensor | Tensor) -> NestedTensor:
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> torch.allclose(torch.mul(nt, 1.5), torch.mul(nt.tensor, 1.5))
        True
    """
    return _binary_op_maybe_tensor(input, other, torch.mul)
    ...
    *,
    ...
    if isinstance(dim, int):
        ...
        for t in input._storage:
            ...
            means.append(m)
        try:
            return torch.stack(vars_), torch.stack(means)
        except (RuntimeError, ValueError, TypeError):
            return NestedTensor(vars_, **input._state), NestedTensor(means, **input._state)
...
def pow(input: NestedTensor, exponent: NestedTensor | Tensor | float) -> NestedTensor:
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 2.0]), torch.tensor([3.0, 4.0]))
        >>> torch.allclose(torch.pow(nt, 1.5), torch.pow(nt.tensor, 1.5))
        True
    """
    return _binary_op_maybe_tensor(input, exponent, torch.pow)
...
def relu(tensor: NestedTensor) -> NestedTensor:
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.relu(nt), torch.relu(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.relu)
...
def sigmoid(tensor: NestedTensor) -> NestedTensor:
    ...
    See also [torch.sigmoid][].

    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.sigmoid(nt), torch.sigmoid(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.sigmoid)
...
def sin(tensor: NestedTensor) -> NestedTensor:
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.sin(nt), torch.sin(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.sin)
...
def sqrt(tensor):
    ...
    Examples:
        >>> import torch
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, 4.0]), torch.tensor([9.0, 16.0]))
        >>> torch.allclose(torch.sqrt(nt), torch.sqrt(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.sqrt)
...
def stack(*args, **kwargs):
    ...
    state = first.__state__(return_dtype=True, return_device=True, return_requires_grad=False)
...
def split(tensor: NestedTensor, split_size_or_sections, dim: int = 0):
    r"""
    ...
    batch_dim = _get_batch_dim(tensor)
    if dim != batch_dim:
        raise NotImplementedError("torch.split for NestedTensor only supports splitting along the batch dimension.")

    if isinstance(split_size_or_sections, int):
        split_size = split_size_or_sections
        if split_size <= 0:
            raise ValueError("split_size must be a positive integer.")
        storage = tensor._storage
        return tuple(
            NestedTensor(storage[i : i + split_size], **tensor._state)  # noqa: E203
    ...
    for section in split_size_or_sections:
        ...
        chunks.append(NestedTensor(storage[start:end], **tensor._state))
        start = end
...
def chunk(input: NestedTensor, chunks: int, dim: int = 0):
    ...
    batch_dim = _get_batch_dim(input)
    if dim != batch_dim:
        raise NotImplementedError("torch.chunk for NestedTensor only supports chunking along the batch dimension.")
...
def tanh(tensor: NestedTensor) -> NestedTensor:
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> nt = NestedTensor(torch.tensor([1.0, -2.0]), torch.tensor([3.0, -4.0]))
        >>> torch.allclose(torch.tanh(nt), torch.tanh(nt.tensor))
        True
    """
    return _concat_apply_same_shape(tensor, torch.tanh)
...
def where(condition, input, other):
    ...
    return NestedTensor(torch.where(c, x, y) for c, x, y in zip(cond_nt._storage, input_nt._storage, other_nt._storage))
