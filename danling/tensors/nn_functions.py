...
def _packed_index_lookup(offsets: Tensor) -> tuple[Tensor, Tensor]:
    r"""Return packed global-index lookup tensors for sequence and local position."""
    lengths = offsets[1:] - offsets[:-1]
    sequence_of_idx = torch.repeat_interleave(torch.arange(lengths.numel(), device=offsets.device), lengths)
    local_of_idx = torch.arange(sequence_of_idx.numel(), device=offsets.device) - offsets[sequence_of_idx]
    return sequence_of_idx, local_of_idx
...
def _flex_wrap_mask_mod(mask_mod: Callable, query: NestedTensor, key: NestedTensor) -> Callable:
    ...
    q_sequence, q_local_lut = _packed_index_lookup(q_offsets)
    k_sequence, k_local_lut = _packed_index_lookup(k_offsets)
...
def _batch_norm_eval_packed_affine(
    ...
    expected = torch.full_like(input._physical_shape[:, 0], num_features)
...
def _pad_packed_variable_last_dim(input: NestedTensor, pad: tuple[int, ...], value=None) -> NestedTensor | None:
    ...
    old_sizes_device = old_sizes.to(device=input._values.device)
    batch_indices = torch.arange(len(input), device=input._values.device).repeat_interleave(
        old_sizes_device,
...
# Tier A is the only compile-safe surface for nn.functional: packed-first
...
# eager mode, but they are not part of the packed compile contract.
