...
from ..ops import _check_execution_guard, _ExecutionGuardKind, _is_compiling, _static_channels_match
...
def _uniform_element_shape(input: NestedTensor) -> tuple[int, ...] | None:
    ...
    if input._element_shapes is not None:
        ...
        rows = input._physical_shape
        if rows.size(0) == 0 or _is_compiling() or _is_fake_tensor(rows):
            return None
        if not torch.equal(rows, rows[:1].expand_as(rows)):
            return None
        shape = tuple(int(size) for size in rows[0].tolist())
...
def _can_use_packed_channel_shuffle_height(input: NestedTensor, groups: int) -> bool:
    ...
    if not _static_channels_match(input._physical_shape, channels):
        return False
