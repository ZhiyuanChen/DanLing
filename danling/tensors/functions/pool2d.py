...
r"""``avg_pool2d`` and ``max_pool2d`` handlers for NestedTensor."""
...
def avg_pool2d(
    ...
    *,
    ...
    output = _packed_avg_pool2d(input, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override)
    if output is not None:
        return output
    return _per_element(
        input,
        F.avg_pool2d if _fn is None else _fn,
        kernel_size,
        stride,
        padding,
        ceil_mode,
...
def max_pool2d(
    ...
    *,
    ...
    fn = F.max_pool2d if _fn is None else _fn
    if not return_indices:
        output = _packed_max_pool2d(input, kernel_size, stride, padding, dilation, ceil_mode)
        if output is not None:
            return output
    if return_indices:
...
def _pool2d_output_meta(
    ...
    for shape in _resolve_element_shapes(input):
        ...
        if out_h <= 0 or out_w <= 0:
            return None
        output_shapes.append((channels, out_h, out_w))
...
def _pool2d_tile_counts(output_shapes: tuple[tuple[int, ...], ...], tile_size: int) -> tuple[int, ...]:
    return tuple((int(shape[1]) * int(shape[2]) + tile_size - 1) // tile_size for shape in output_shapes)


def _make_pool2d_metadata(
    input: NestedTensor,
...
if triton is not None:
    ...
    @triton.jit
    def _avg_pool2d_forward_kernel(
        input_ptr,
        output_ptr,
        input_offsets_ptr,
        output_offsets_ptr,
        ...
        for kernel_y in range(0, kernel_h):
            ...
            for kernel_x in range(0, kernel_w):
                ...
                valid = (out_flat < out_hw) & valid_y & (in_x >= 0) & (in_x < in_w)
        ...
        if divisor_override > 0:
            ...
            divisor = kernel_h * kernel_w + tl.zeros((block_l,), dtype=tl.float32)
        else:
            divisor = valid_count
        result = acc / divisor[:, None]
        output_mask = (out_flat[:, None] < out_hw) & (offsets_c[None, :] < channels)
        tl.store(
    ...
    @triton.jit
    def _avg_pool2d_backward_kernel(
        grad_output_ptr,
        grad_input_ptr,
        input_offsets_ptr,
        output_offsets_ptr,
        ...
        grad = (
            tl.load(
                ...
                mask=(out_flat[:, None] < out_hw) & (offsets_c[None, :] < channels),
                other=0.0,
            )
            / divisor[:, None]
        )
    ...
    @triton.jit
    def _max_pool2d_forward_kernel(
        input_ptr,
        output_ptr,
        indices_ptr,
        input_offsets_ptr,
    ...
    @triton.jit
    def _max_pool2d_backward_kernel(
        input_ptr,
        grad_output_ptr,
        grad_input_ptr,
        input_offsets_ptr,
        ...
        grad_mask = (out_flat[:, None] < out_hw) & (offsets_c[None, :] < channels)
        tl.atomic_add(grad_input_ptrs, grad, sem="relaxed", mask=grad_mask)

    @triton.jit
    def _max_pool2d_backward_indices_kernel(
        grad_output_ptr,
        grad_input_ptr,
        indices_ptr,
        input_offsets_ptr,
    ...
    _avg_pool2d_forward_kernel = None
    _avg_pool2d_backward_kernel = None
    _max_pool2d_forward_kernel = None
    _max_pool2d_backward_kernel = None
    _max_pool2d_backward_indices_kernel = None


class _PackedAvgPool2dFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        input_values: Tensor,
        ...
        _avg_pool2d_forward_kernel[grid](
            input_values,
            output,
            input_offsets,
            output_offsets,
        ...
        ctx.non_overlapping = stride_h >= kernel_h and stride_w >= kernel_w
        return output
    ...
    def backward(ctx, grad_output: Tensor):
        ...
        _avg_pool2d_backward_kernel[grid](
            grad_output,
            grad_input,
            input_offsets,
            output_offsets,
...
class _PackedMaxPool2dFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        input_values: Tensor,
        ...
        output = input_values.new_empty((output_size, channels))
        max_input_size = int((shape_meta[:, 0] * shape_meta[:, 1]).max().item())
        save_indices = bool(input_values.requires_grad and max_input_size < 2**31)
        indices = (
        ...
        block_l, block_c = _pool_block_size()
        grid = (tile_count, triton.cdiv(channels, block_c))
        _max_pool2d_forward_kernel[grid](
            input_values,
            output,
            indices,
            input_offsets,
    ...
    def backward(ctx, grad_output: Tensor):
        ...
        if ctx.use_indices:
            ...
            grad_input = grad_output.new_zeros(ctx.input_shape)
            _max_pool2d_backward_indices_kernel[grid](
                grad_output,
                grad_input,
                indices,
                input_offsets,
...
def _packed_avg_pool2d(
    ...
    if ceil_mode or not _can_use_packed_pool2d(input):
        return None
    ...
    stride_pair = (stride_value, stride_value)
    padding_pair = (int(padding), int(padding))
    divisor = 0 if divisor_override is None else int(divisor_override)
    if any(value <= 0 for value in (*kernel, *stride_pair)):
        return None
    if min(padding_pair) < 0 or padding_pair[0] > kernel[0] // 2 or padding_pair[1] > kernel[1] // 2:
        return None
    if divisor <= 0 and divisor_override is not None:
        return None
    output_meta = _pool2d_output_meta(input, kernel, stride_pair, padding_pair)
...
def _packed_max_pool2d(
    ...
    if any(value <= 0 for value in (*kernel, *stride_pair, *dilation_pair)):
        return None
    if min(padding_pair) < 0 or padding_pair[0] > kernel[0] // 2 or padding_pair[1] > kernel[1] // 2:
        return None
    output_meta = _pool2d_output_meta(input, kernel, stride_pair, padding_pair, dilation_pair)
    if output_meta is None:
        return None
    output_shapes, output_packed_sizes, output_shape_tensor = output_meta
    output_offsets = type(input)._offsets_from_sizes(output_packed_sizes, dtype=torch.long)
