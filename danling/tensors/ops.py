...
def _binary_complementary_singleton_rectangular(lhs, rhs, op, extra_args, extra_kwargs):
    r"""Form adjacent rectangular products from independent eager row/column lengths.
    ...
    The existing square graph path is separate; this route needs concrete
    metadata and does not add compiled rectangular support.
    """
    from .aten_functions import _is_fake_tensor
    ...
    prefix_rank = row._ragged_dims[0]
    row_values = row.concat.select(prefix_rank + 1, 0).index_select(0, row_index)
    column_values = column.concat.select(prefix_rank + 1, 0).index_select(0, column_index)
