...
if hasattr(getattr(torch, "compiler", None), "is_compiling"):
    _torch_is_compiling = torch.compiler.is_compiling
else:  # pragma: no cover - PyTorch releases without torch.compiler
...
def _is_compiling() -> bool:
    r"""Return whether a ``torch.compile``/``torch.export`` session is active.

    The session flag also covers Python that runs for real on fake tensors outside
    Dynamo-traced frames: ``__torch_function__`` handlers during fake propagation and
    ``__torch_dispatch__`` during AOTAutograd. ``is_dynamo_compiling()`` is only rewritten
    to ``True`` inside traced frames and would leave the compile-safety gates dormant there.
    """
...
def _per_element_unsupported(nested: NestedTensor, op_name: str) -> None:
    r"""Reject a per-sample replay whose packed sizes cannot be recovered under tracing."""
    # Dynamo fake-propagates eager-only handlers outside traced frames, where
    # _is_compiling() is False; tensor-backed fake layouts carry no Python packed sizes.
    from .aten_functions import _is_fake_tensor

    if _is_compiling() or (nested._packed_sizes is None and _is_fake_tensor(nested._offsets)):
        _compile_unsupported(op_name, "the per-element fallback replays the operator once per sample")


def _static_channels_match(physical_shape: Tensor, channels: int) -> bool:
    r"""Return whether every element has ``channels`` leading entries; ``False`` when metadata is not concrete."""
    from .aten_functions import _is_fake_tensor

    # Comparing fake metadata is data-dependent; callers treat False as "packed path ineligible".
    if _is_compiling() or _is_fake_tensor(physical_shape):
        return False
    return bool(torch.equal(physical_shape[:, 0], torch.full_like(physical_shape[:, 0], int(channels))))


def _uniform_physical_extent(physical_shape: Tensor, dim: int) -> int | None:
    r"""Return the size every element shares along physical ``dim``; ``None`` if they differ or are not concrete."""
    from .aten_functions import _is_fake_tensor

    if _is_compiling() or _is_fake_tensor(physical_shape) or physical_shape.size(0) == 0:
        return None
    column = physical_shape[:, dim]
    extent = int(column[0])
    return extent if bool(torch.equal(column, torch.full_like(column, extent))) else None
...
def _packed_static_extents(nt: NestedTensor) -> tuple[tuple[int, int], ...]:
    ...
    return tuple((dim, int(nt.concat.shape[1 + axis])) for axis, dim in enumerate(nt._static_dims))
...
def _dense_alignment_to_values(nt: NestedTensor, other: Tensor, reading: _DenseReading) -> Tensor | None:
    r"""Rewrite an aligned dense operand into the packed axis order of ``nt.concat``."""
...
def _can_broadcast_nested_to(
    ...
    *,
    ...
    dimensions, retain a ragged level whose per-sample extent is one, or retain
    every ragged dimension in another order. Its remaining static dimensions may be
    ...
    for dim in omitted_ragged:
        source_extent = source.concat.shape[1 + source_static.index(dim)]
    ...
    for dim in target_static:
        source_extent = source.concat.shape[1 + source_static.index(dim)]
        target_extent = target.concat.shape[1 + target_static.index(dim)]
...
def _has_retained_ragged_singleton_source(target: NestedTensor, source: NestedTensor) -> bool:
    r"""Choose a packed broadcast target using globally singleton ragged extents.

    A narrowed shared mask retains its ragged declarations. Its known singleton
    extent distinguishes it from the full operand before same-structure checks
    can assert equal offsets under compilation. Cases requiring a new topology
    assembled from both operands remain outside this directional fast path.
    """
    ...
    for dim in source._ragged_dims:
        source_extent = source._logical_shape[_logical_dim_for_element_dim(source, dim)]
        target_extent = target._logical_shape[_logical_dim_for_element_dim(target, dim)]
        source_singleton = statically_known_true(source_extent == 1)
        if statically_known_true(target_extent == 1) and statically_known_true(source_extent != 1):
            return False
        has_singleton = has_singleton or source_singleton
...
def _broadcast_condition_matches(condition, *operands: NestedTensor) -> bool:
    ...
    if statically_known_true(condition):
        return True
    if _is_compiling() or any(_is_fake_tensor(operand.concat) for operand in operands):
    ...
    *,
    ...
    if len(target) == 0 and len(source) == 0:
        ...
        if any(not _broadcast_condition_matches(source_extents[dim] == 1, target, source) for dim in omitted_ragged):
            return False
        if any(
            not _broadcast_condition_matches(
                (source_extents[dim] == 1) | (target_extents[dim] == source_extents[dim]), target, source
            )
            for dim in source._ragged_dims
        ):
            return False
    ...
    conditions.extend(
        torch.all((source_shape.select(1, dim) == 1) | (target_shape.select(1, dim) == source_shape.select(1, dim)))
        for dim in source_ragged
    )
    for dim in target_static:
        source_sizes = source_shape.select(1, dim)
        target_sizes = target_shape.select(1, dim)
        compatible_sizes = (source_sizes == 1) | (target_sizes == source_sizes)
        if allow_target_static_broadcast:
            compatible_sizes = compatible_sizes | (target_sizes == 1)
        conditions.append(torch.all(compatible_sizes))
    if not conditions:
        return True
    ...
    if _is_compiling() or _is_fake_tensor(valid):
        torch._assert_async(
            ...
            "singleton or matching retained ragged dimensions, and singleton or matching static dimensions",
        )
        return True
    return bool(valid)
...
def _broadcast_nested_to_values(
    ...
    *,
    ...
    r"""Align ``source`` with ``target.concat`` without padding or storage mapping."""
    ...
    for dim in source._ragged_dims:
        ...
        coordinate = torch.where(radix == 1, 0, coord_by_dim[dim])
        source_local = source_local * radix + coordinate
    ...
    values = source.concat.index_select(0, source_indices)
...
def _broadcast_lower_rank_nested_to_values(target: NestedTensor, source: NestedTensor) -> Tensor | None:
    r"""Rewrite ``source.concat`` into ``target``'s packed tail without unpacking either."""
    ...
    for dim in target._static_dims:
        ...
        if origin in source_static:
            ...
            axes.append(axis)
            tail.append(int(source.concat.shape[axis]))
        else:
            tail.append(1)
    values = source.concat.permute((0, *axes))
    return values.reshape(values.shape[0], *tail)


def _hint_complementary_product_size(
    total: int, total_rows: int | None, total_columns: int | None, batch_size: int
) -> None:
    r"""Advise compiler scheduling from the input extents, without shape guards.

    The product of the mean axis lengths estimates a typical pair extent. It is
    neither the exact sum of per-sample products nor a bound for rectangular
    inputs: correlations between their row and column lengths can move the
    actual size in either direction. This affects optimization choices only.
    Only input extents with existing backed hints contribute; data-dependent
    input sizes leave the compiler's existing policy unchanged.
    """
    if total_rows is None or total_columns is None or not hasattr(torch._dynamo, "override_optimization_hint"):
        return
    from torch.fx.experimental import symbolic_shapes

    if not hasattr(symbolic_shapes, "optimization_hint") or not hasattr(symbolic_shapes, "has_guarding_hint"):
        return

    def input_hint(value: int) -> int:
        # Checking for an existing backed hint adds no guard. Do not mistake
        # an unbacked symbol's lower bound for an observed typical input size.
        if isinstance(value, torch.SymInt) and not symbolic_shapes.has_guarding_hint(value):
            return 0
        return symbolic_shapes.optimization_hint(value, fallback=0)

    row_hint, column_hint, batch_hint = (input_hint(value) for value in (total_rows, total_columns, batch_size))
    if row_hint > 0 and column_hint > 0 and batch_hint > 0:
        torch._dynamo.override_optimization_hint(total, max(1, row_hint * column_hint // batch_hint))


@torch.library.custom_op("danling::_complementary_square_indices", mutates_args=())
def _complementary_square_indices(
    lengths: Tensor, device: torch.device | None = None, total_rows: int | None = None
) -> tuple[Tensor, Tensor]:
    r"""Build row-major packed indices for per-sample square products."""
    del total_rows  # Optional shape evidence is consumed only by the fake implementation.
    ...
    square_sizes = square_sizes.to(device=device)
    offsets = offsets.to(device=device)
    square_offsets = square_offsets.to(device=device)
    batch = torch.repeat_interleave(
        torch.arange(lengths.numel(), device=device),
        square_sizes,
        output_size=total,
    )
    local = torch.arange(total, device=device) - square_offsets[batch]
    width = lengths[batch]
...
def _complementary_square_indices_fake(
    lengths: Tensor, device: torch.device | None = None, total_rows: int | None = None
) -> tuple[Tensor, Tensor]:
    ctx = torch.library.get_ctx()
    total = ctx.new_dynamic_size(min=0)
    _hint_complementary_product_size(total, total_rows, total_rows, lengths.numel())
    index = lengths.new_empty((total,), dtype=torch.long, device=device)
    return index, index.new_empty(index.shape)


@torch.library.custom_op("danling::_complementary_rectangular_indices", mutates_args=())
def _complementary_rectangular_indices(
    rows: Tensor,
    columns: Tensor,
    device: torch.device,
    ...
    r"""Build row-major indices on the values device from small CPU partitions."""
    del total_rows, total_columns  # The runtime result always follows the actual CPU partitions.
    rows, columns = rows.to(dtype=torch.long), columns.to(dtype=torch.long)
    counts = rows * columns
    total = int(counts.sum().item())
    row_offsets = torch.nn.functional.pad(rows.cumsum(0), (1, 0))
    column_offsets = torch.nn.functional.pad(columns.cumsum(0), (1, 0))
    pair_offsets = torch.nn.functional.pad(counts.cumsum(0), (1, 0))
    # Transfer O(batch) metadata, not two O(pair cells) index buffers. The
    # concrete custom-op boundary already knows total without a device sync.
    row_offsets = row_offsets.to(device=device)
    column_offsets = column_offsets.to(device=device)
    pair_offsets = pair_offsets.to(device=device)
    columns = columns.to(device=device)
    counts = counts.to(device=device)
    batch = torch.repeat_interleave(torch.arange(rows.numel(), device=device), counts, output_size=total)
    local = torch.arange(total, device=device) - pair_offsets[batch]
    width = columns[batch]
    return row_offsets[batch] + torch.div(local, width, rounding_mode="floor"), column_offsets[batch] + local.remainder(
        width
    )


@_complementary_rectangular_indices.register_fake
def _complementary_rectangular_indices_fake(
    rows: Tensor,
    columns: Tensor,
    device: torch.device,
    total_rows: int | None = None,
    ...
    _hint_complementary_product_size(total, total_rows, total_columns, rows.numel())
    index = rows.new_empty((total,), dtype=torch.long, device=device)
    return index, index.new_empty(index.shape)
...
def _complementary_singleton_square_operands(
    ...
    *,
    ...
    require_matching_lengths: bool = True,
    ...
    if row._permutation != row_order or column._permutation != column_order:
        return None
    if row.concat.dim() != rank or column.concat.dim() != rank:
        return None
    singleton_axis = row_dim + 1
    if row.concat.shape[singleton_axis] != 1 or column.concat.shape[singleton_axis] != 1:
        return None
    ...
    if (
        ...
        and row_sizes is not None
        and column_sizes is not None
        and tuple(row_sizes) != tuple(column_sizes)
    ):
        return None
    return row, column
...
def _binary_complementary_singleton_square(
    ...
    *,
    ...
    if fake and not graph_mode and retained_sizes is not None:
        ...
        row_index = row.concat.new_empty((total,), dtype=torch.long)
        column_index = column.concat.new_empty((total,), dtype=torch.long)
    else:
        if not _has_same_ragged_structure(row, column):
            return None
        offsets = row.packed_offsets(dtype=torch.long)
        lengths = offsets[1:] - offsets[:-1]
        row_index, column_index = _complementary_square_indices(lengths, row.concat.device, row.concat.shape[0])
        row_index = row_index.to(device=row.concat.device)
        column_index = column_index.to(device=column.concat.device)

    prefix_rank = row._ragged_dims[0]
    singleton_axis = prefix_rank + 1
    row_values = row.concat.select(singleton_axis, 0)[row_index]
    column_values = column.concat.select(singleton_axis, 0)[column_index]
...
def _binary_complementary_singleton_rectangular(lhs, rhs, op, extra_args, extra_kwargs):
    r"""Form adjacent rectangular products from independent row/column lengths.

    Neither operand alone describes the result's two ragged axes. Gather both
    operands into output-sized row-major storage and retain both declarations.
    The integer index custom op exposes one data-dependent output extent to
    tracing; gradients still use ordinary differentiable indexing operations.
    """
    from .aten_functions import _is_fake_tensor

    operands = _complementary_singleton_square_operands(
        lhs, rhs, allow_static_prefix=True, require_matching_lengths=False
    )
    if operands is None:
        return None
    row, column = operands
    row_offsets, column_offsets = row.packed_offsets(), column.packed_offsets()
    ...
    if _is_compiling() or fake:
        ...
        row_index = row_index.to(device=device)
        column_index = column_index.to(device=column.concat.device)
    else:
        counts = rows * columns
        total = int(counts.sum().item())
        counts_device = counts.to(device=device)
        batch = torch.repeat_interleave(torch.arange(len(row), device=device), counts_device, output_size=total)
        starts = torch.nn.functional.pad(counts.cumsum(0), (1, 0)).to(device=device)
        local = torch.arange(total, device=device) - starts[batch]
        width = columns.to(device=device)[batch]
        row_index = row_offsets.to(device=device)[batch] + torch.div(local, width, rounding_mode="floor")
        column_index = column_offsets.to(device=device)[batch] + local.remainder(width)
    prefix_rank = row._ragged_dims[0]
    row_values = row.concat.select(prefix_rank + 1, 0)[row_index]
    column_values = column.concat.select(prefix_rank + 1, 0)[column_index]
    left, right = (row_values, column_values) if lhs is row else (column_values, row_values)
    values = op(left, right, *extra_args, **extra_kwargs)
    result = row.packed_with_rectangular_lengths(values, rows, columns)
    result = _restore_complementary_square_prefix(result, prefix_rank)
    return result._packed_like_unchecked(values) if values.requires_grad else result
...
def _single_element_logical_view(input: NestedTensor) -> Tensor:
    ...
    packed_view = input.concat.reshape(packed_shape)
...
def _binary_single_element_nested_broadcast(
    ...
    *,
    ...
    if len(target) != 1 or len(source) != 1 or _is_compiling():
        return None
    if _is_fake_tensor(target.concat) or _is_fake_tensor(source.concat):
        return None
    if not _can_broadcast_nested_to(target, source, allow_target_static_broadcast=True):
        return None
    ...
    if packed_result.shape[1:] == target.concat.shape[1:]:
...
def _binary_op_maybe_tensor(input, other, op, *extra_args, **extra_kwargs):
    ...
    def _rebuild_from(reference, values):
        if tuple(values.shape[1:]) != tuple(reference.concat.shape[1:]):
            if values.dim() == reference.concat.dim():
    ...
    if len(input) == 0 and not isinstance(other, cls):
        resolved = _as_tensor_like(other, input.concat)
        new_values = (
            op(resolved, input.concat, *extra_args, **extra_kwargs)
            if reverse
            else op(input.concat, resolved, *extra_args, **extra_kwargs)
        )
        return _rebuild(new_values)

    # NT + scalar or 0-d tensor (most common in training)
    if not isinstance(other, Tensor) or other.dim() == 0:
        val = _as_tensor_like(other, input.concat)
        new_values = (
            op(val, input.concat, *extra_args, **extra_kwargs)
            if reverse
            else op(input.concat, val, *extra_args, **extra_kwargs)
        )
    ...
    if not isinstance(other, cls):
        ...
        if resolved is not None:
            lhs, rhs = (resolved, input.concat) if reverse else (input.concat, resolved)
    ...
    if isinstance(other, cls):
        ...
        for target, source, target_is_input in ((input, other, True), (other, input, False)):
            if not _has_retained_ragged_singleton_source(target, source):
                continue
            aligned = _broadcast_nested_to_values(target, source, allow_target_static_broadcast=True)
            if aligned is not None:
                lhs, rhs = (target.concat, aligned) if target_is_input else (aligned, target.concat)
                if reverse:
                    lhs, rhs = rhs, lhs
                return _rebuild_from(target, op(lhs, rhs, *extra_args, **extra_kwargs))
        if input._has_same_structure(other):
            ...
            if new_values.shape[1:] == input.concat.shape[1:]:
                return input._packed_like_unchecked(new_values)
            return _packed_with_static_tail_from_values(input, new_values)

        # Independent partitions may coincide on this batch and differ on the
        # next. Do not install the square path's equality assertion just to
        # decide whether that specialization applies. Shared offsets remain a
        # structurally known square; otherwise trace the rectangular product.
        if input._offsets is not other._offsets and (
            _is_compiling() or _is_fake_tensor(input.concat) or _is_fake_tensor(other.concat)
        ):
            rectangular = _binary_complementary_singleton_rectangular(input, other, op, extra_args, extra_kwargs)
        ...
        if aligned_other is not None:
            new_values = op(input.concat, aligned_other, *extra_args, **extra_kwargs)
        ...
        if aligned_input is not None:
            new_values = op(aligned_input, other.concat, *extra_args, **extra_kwargs)
            return _rebuild_from(other, new_values)

        aligned_other = _broadcast_lower_rank_nested_to_values(input, other)
        if aligned_other is not None:
            lhs, rhs = (aligned_other, input.concat) if reverse else (input.concat, aligned_other)
            return _rebuild(op(lhs, rhs, *extra_args, **extra_kwargs))

        aligned_input = _broadcast_lower_rank_nested_to_values(other, input)
        if aligned_input is not None:
            lhs, rhs = (other.concat, aligned_input) if reverse else (aligned_input, other.concat)
            new_values = op(lhs, rhs, *extra_args, **extra_kwargs)
            if new_values.shape[1:] == other.concat.shape[1:]:
    ...
    raise NotImplementedError(
        ...
        f"broadcast-compatible with packed values: values shape {input.concat.shape}, tensor shape {other.shape}"
    )
    ...
    if isinstance(other, NestedTensor):
        ...
        if (
            _complementary_singleton_square_operands(
                input, other, allow_static_prefix=True, require_matching_lengths=False
            )
            is not None
        ):
            return True
...
def _concat_apply_same_shape(input: NestedTensor, op: Callable[[Tensor], Tensor]):
    ...
    return input._packed_like_unchecked(op(input.concat))
...
def _concat_dim_for_tensor_dim(input: NestedTensor, dim: int) -> int | None:
    ...
    if input._element_shapes is not None:
        ...
        # The declared layout is exact: a ragged dim shares the packed axis, every other dim
        # owns the packed axis its position in the permutation gives it.
        rank = int(st.size(1))
        if dim < 0:
            dim += rank
        if dim < 0 or dim >= rank:
            raise IndexError(f"Dimension out of range for NestedTensor with {rank} dims: {dim}")
        if dim in input._ragged_dims:
            return None
        return 1 + input._static_dims.index(dim)
    else:
        if _is_compiling():
...
def _reduce_none(input: NestedTensor, op, *, dtype: torch.dtype | None = None, keepdim: bool = False, **op_kwargs):
    ...
    if dtype is not None:
        op_kwargs["dtype"] = dtype
    result = op(input.concat.reshape(-1), **op_kwargs)
    if keepdim:
...
def _reduce_none_pair(input: NestedTensor, op, *, dtype: torch.dtype | None = None, keepdim: bool = False, **op_kwargs):
    ...
    if dtype is not None:
        op_kwargs["dtype"] = dtype
    a, b = op(input.concat.reshape(-1), **op_kwargs)
    if keepdim:
        shape = (1,) * input.dim()
...
def _packed_normalize_axes(input: NestedTensor, normalized_shape: tuple[int, ...]) -> tuple[int, ...] | None:
    r"""Return the packed ``_values`` axes that carry the normalized trailing dimensions.

    ``normalized_shape`` names the trailing *logical* element dimensions, while the
    normalization kernels reduce the trailing axes of ``_values``, which are in packed
    permutation order. The two coincide only for an identity layout, so resolve every
    named dimension through the packed layout instead of assuming they line up. Returns
    ``None`` when a named dimension is ragged -- collapsed into packed axis 0, so it has
    no packed axis of its own -- or when its extent disagrees with ``normalized_shape``.
    """
    ...
    if ndim > rank:
        return None
    axes = []
    for offset, physical_dim in enumerate(range(rank - ndim, rank)):
        values_dim = _physical_to_values_dim(input, physical_dim)
        if values_dim is None:
            return None
        if int(input.concat.shape[values_dim]) != int(normalized_shape[offset]):
            return None
        axes.append(values_dim)
    return tuple(axes)


def _packed_normalize(input: NestedTensor, normalized_shape: tuple[int, ...], run: Callable[[Tensor], Tensor]):
    r"""Apply a normalization to packed values, moving the named axes into reducing position.

    A permuted layout keeps the named dimensions on packed storage but not at the end of
    it, so rotate them to the trailing axes for the kernel and rotate the result back. The
    identity case is the common one and takes no copy at all.
    """
    axes = _packed_normalize_axes(input, normalized_shape)
    if axes is None:
        return None
    values = input.concat
    target = tuple(range(values.dim() - len(axes), values.dim()))
    if axes == target:
        return input._packed_like_unchecked(run(values))
    return input._packed_like_unchecked(run(values.movedim(axes, target)).movedim(target, axes))
...
def _packed_layer_norm(
    ...
    r"""Run layer norm on packed ``_values`` when the normalized dimensions are static."""

    def run(values: Tensor) -> Tensor:
        output, _, _ = torch.ops.aten.native_layer_norm.default(values, normalized_shape, weight, bias, eps)
        return output

    return _packed_normalize(input, normalized_shape, run)


def _packed_rms_norm(input: NestedTensor, normalized_shape: tuple[int, ...], weight: Tensor | None, eps):
    r"""Run RMS norm on packed ``_values`` when the normalized dimensions are static."""

    def run(values: Tensor) -> Tensor:
        return torch.ops.aten.rms_norm.default(values, normalized_shape, weight, eps)
...
def _map_storage_pair(input: NestedTensor, op, *args, **kwargs):
    ...
    if len(input) == 0:
        try:
            first_probe, second_probe = op(input.concat, *args, **kwargs)
