...
def _maybe_align_dense_to_nested(ref: NestedTensor, value) -> NestedTensor | None:
    ...
    converted, and the conversion always uses ``nested_like(..., strict=False)``.
    """
    cls = type(ref)
...
def _binary_op_maybe_tensor(input, other, op, *extra_args, **extra_kwargs):
    ...
    """
    from .aten_functions import _packed_like, _structure_match
    from .nested_tensor import NestedTensor
    ...
    if isinstance(other, cls):
        ...
        if _structure_match(input, other):
...
def _concat_dim_for_tensor_dim(input: NestedTensor, dim: int) -> int | None:
    ...
    if input._physical_shape.numel() == 0:
        return None
