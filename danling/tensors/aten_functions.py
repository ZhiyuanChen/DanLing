...
from torch._C._functorch import is_functorch_wrapped_tensor as _is_functorch_wrapped_tensor
...
from .ops import (
    ...
    _restore_complementary_square_prefix,
    _stack_or_nest,
    _translate_dim,
    _translate_dims,
    _uniform_physical_extent,
)
...
def _is_fake_tensor(tensor: Tensor) -> bool:
    # A plain dense tensor can only hide a fake tensor behind a functorch or C++ functional
    # wrapper; those two checks are all PyTorch's ``is_fake`` does for it as well.
    if type(tensor) is Tensor and not torch._is_functional_tensor(tensor) and not _is_functorch_wrapped_tensor(tensor):
        return False
...
def per_element_fallback(func, args, kwargs):
    ...
    if len(source) == 0:
        ...
        try:
            ...
            return rebuild_empty(source.concat)
...
def _resolve_other(source, other, func):
    ...
    Returns the value to use alongside ``source.concat``:
    - NestedTensor with matching packed layout → ``other.concat``
...
def _elementwise_binary_handler(func, args, kwargs):
    ...
    if isinstance(lhs, NestedTensor):
        ...
        return lhs._packed_like_unchecked(func(lhs.concat, resolved, *extra, **kwargs))
    resolved = _resolve_other(rhs, lhs, func)
    return rhs._packed_like_unchecked(func(resolved, rhs.concat, *extra, **kwargs))
...
def _elementwise_unary_handler(func, args, kwargs):
    ...
    source = args[0]
    return source._packed_like_unchecked(func(source.concat, *args[1:], **kwargs))
...
def _global_reduction_handler(func, args, kwargs):
    ...
    source = args[0]
    return func(source.concat, **kwargs)
...
def _dim_reduction_dispatch(func, source, dims, keepdim, kwargs, *, ragged_fill, keepdim_kw=False, none_dim):
    ...
    if len(dims) > 1:
        ...
        if all(dim_i is not None for dim_i in values_dims):
            return _reduce_non_ragged_packed_dims(
                source,
                _call(source.concat, [int(dim_i) for dim_i in values_dims], keepdim),
                dims_adj,
                keepdim,
            )
    ...
    if values_dim is not None:
        return _reduce_non_ragged_packed(
            source,
            _call(source.concat, [values_dim], keepdim),
            dim_adj,
            keepdim,
        )
    ...
    return _reduce_non_ragged_packed(source, _call(source.concat, [dim_adj], keepdim), dim_adj, keepdim)
...
def count_nonzero_dim_reduction(func, args, kwargs):
    ...
    if len(dims) > 1:
        ...
        values_dims = tuple(_physical_to_values_dim(source, dim_i) for dim_i in dims_adj)
        if all(dim_i is not None for dim_i in values_dims):
            out_values = func(source.concat, list(cast(tuple[int, ...], values_dims)), **kwargs)
    ...
    out_values = func(source.concat, [values_dim], **kwargs)
...
def _order_stat_pair_reduction(source: NestedTensor, dim: int, keepdim: bool, apply):
    ...
    values_out, indices_out = apply(source.concat, dim_adj, keepdim)
    return (
...
def linalg_vector_norm(func, args, kwargs):
    ...
    out_values = func(source.concat, ord_value, [values_dim], keepdim, dtype=dtype, **kwargs)
    return _reduce_non_ragged_packed(source, out_values, dim_adj, keepdim)
...
def var_mean_dim_reduction(func, args, kwargs):
    ...
    out_var, out_mean = func(source.concat, [values_dim], keepdim=keepdim, **kwargs)
    return (
...
def _per_element_true_counts(mask: NestedTensor) -> Tensor:
    ...
    if len(mask) == 0:
        return torch.empty((0,), dtype=torch.long, device=mask.concat.device)
    if mask.concat.dim() > 1:
        row_counts = mask.concat.reshape(mask.concat.shape[0], -1).to(dtype=torch.long).sum(dim=1)
    else:
        row_counts = mask.concat.to(dtype=torch.long)
...
def _masked_scatter_supply_suffices(mask: NestedTensor, source: NestedTensor) -> bool:
    ...
    if len(mask) != len(source):
        return False
    if _is_fake_tensor(mask.concat) or _is_fake_tensor(source.concat):
        return True
    source_numel = _per_element_numel(source).to(device=mask.concat.device)
...
def _packed_masked_scatter(input: NestedTensor, mask: NestedTensor, source: NestedTensor) -> Tensor:
    ...
    values = input.concat
    mask_values = mask.concat.to(device=values.device)
    source_values = source.concat.to(device=values.device, dtype=values.dtype)
...
def _packed_masked_fill_supported(source: NestedTensor, mask: NestedTensor) -> bool:
    ...
    if not source._has_same_structure(mask):
        return False
    if source.concat.dim() != mask.concat.dim():
        return False
    ...
    return all(
        ...
        for source_size, mask_size in zip(source.concat.shape[1:], mask.concat.shape[1:])
    )
...
def _masked_fill_handler(func, args, kwargs):
    ...
    if isinstance(mask, NestedTensor) and _packed_masked_fill_supported(source, mask):
        return source._packed_like_unchecked(func(source.concat, mask.concat, value, **kwargs))
    ...
    if not isinstance(mask, Tensor):
        mask = torch.as_tensor(mask, dtype=torch.bool, device=source.concat.device)
...
def masked_select(func, args, kwargs):
    ...
    if _is_fake_tensor(source.concat) or _is_fake_tensor(mask.concat):
    ...
    mask_values = mask.concat
    if mask_values.device != source.concat.device:
        mask_values = mask_values.to(device=source.concat.device)
    out_values = func(source.concat, mask_values, **kwargs)
...
def nonzero(func, args, kwargs):
    ...
    if _is_fake_tensor(source.concat):
        raise NotImplementedError(f"NestedTensor: {func} requires concrete values")
    if source._physical_shape.size(1) > 1 and source.concat.dim() == 1:
    ...
    packed_indices = func(source.concat, **kwargs)
...
def copy(func, args, kwargs):
    ...
    if isinstance(src, NestedTensor) and dest._has_same_layout(src):
        func(dest.concat, src.concat, *args[2:], **kwargs)
...
def _inplace_unary_handler(func, args, kwargs):
    ...
    source = args[0]
    func(source.concat, *args[1:], **kwargs)
...
def _inplace_binary_handler(func, args, kwargs):
    ...
    func(source.concat, resolved, *args[2:], **kwargs)
    source._invalidate_transient_caches()
    return source
    ...
    if isinstance(index, NestedTensor):
        ...
        if _single_ragged_gather_supported(source, index, dim_adj):
            ...
            packed_index = _segmented_row_index(
                ...
                dim,
                "gather",
                row_batch_indices=index.packed_batch_indices(device=source.concat.device),
            )
...
def _single_ragged_gather_supported(source: NestedTensor, index: NestedTensor, dim_adj: int) -> bool:
    ...
    return (
        ...
        and source.concat.dim() == index.concat.dim()
...
def _shared_index_gather_supported(source: NestedTensor, dim_adj: int, index: Tensor) -> bool:
    ...
    if index.dim() != rank or len(source) == 0:
        return False
    if _is_fake_tensor(source.concat) or _is_fake_tensor(source._offsets):
        return False
...
def _shared_index_gather(source: NestedTensor, dim, dim_adj: int, index: Tensor, func, sparse_grad, kwargs):
    ...
    if values_dim is not None:
        ...
        out_values = func(
            source.concat.index_select(0, rows), values_dim, packed_index, sparse_grad=sparse_grad, **kwargs
        )
    else:
        ...
        out_values = func(
            source.concat, 0, align_rows(starts, packed_index) + packed_index, sparse_grad=sparse_grad, **kwargs
        )
...
def _gather_equivalent_index(source: NestedTensor, index: NestedTensor, dim_adj: int) -> bool:
    ...
    """
    if source.concat.dim() != index.concat.dim():
        return False
    ...
    return all(
        ...
        for axis, (size, other) in enumerate(zip(source.concat.shape, index.concat.shape))
    )
...
def _index_write_like(source, dim, index, src, apply_fn, op_name: str):
    ...
    index_values = index.to(device=source.concat.device, dtype=torch.long)
    if (
        isinstance(src, NestedTensor)
        and dim_adj > 0
        and source.concat.dim() > dim_adj
        ...
        src_values = src.concat
        if src_values.device != source.concat.device:
            src_values = src_values.to(device=source.concat.device)
        return source._packed_like_unchecked(apply_fn(source.concat, dim_adj, index_values, src_values))
...
def _packed_index_put_indices(source: NestedTensor, indices):
    ...
    for dim, idx in enumerate(broadcasted[1:], start=1):
        dim_size = source.concat.shape[dim]
...
def _packed_index_put_values(source: NestedTensor, values, broadcast_shape, indexed_dims: int):
    ...
    trailing_shape = tuple(source.concat.shape[indexed_dims:])
    batch = len(source)
    expanded_items = batch * math.prod(broadcast_shape)

    if isinstance(values, Tensor) and not isinstance(values, NestedTensor):
        if values.device != source.concat.device:
            values = values.to(device=source.concat.device)
    ...
    expected_sizes = torch.cat((expected_prefix, expected_suffix), dim=1)
    if not NestedTensor._meta_tensor_equal(values._physical_shape, expected_sizes):
        return None

    value_tensor = values.concat
    if value_tensor.device != source.concat.device:
        value_tensor = value_tensor.to(device=source.concat.device)
...
def index_put(func, args, kwargs):
    ...
    if packed is not None:
        ...
        if value_tensor is not None:
            return source._packed_like_unchecked(func(source.concat, packed_indices, value_tensor, accumulate))
...
def index_select(func, args, kwargs):
    ...
    dim = _normalize_dim(dim, source.dim())
    batch_dim = _get_batch_dim(source)
    index_device = index.to(device=source.concat.device, dtype=torch.long)

    if dim == batch_dim:
        # Only batch selection needs index values on the CPU to select layout
        # metadata. Static/inner-ragged selection consumes the device index.
        ...
        offsets_dev = source.packed_offsets(device=source.concat.device, dtype=torch.long)
        starts = offsets_dev.index_select(0, index_device)
        picked_offsets = out_offsets.to(device=source.concat.device, dtype=torch.long)
        # The host layout already determines the exact extent; do not fetch a
        # CUDA scalar or recompute its cumulative lengths to allocate indices.
        total = int(out_offsets[-1].item())
        flat = torch.arange(total, device=source.concat.device, dtype=torch.long)
        if flat.numel() == 0:
            out_values = source.concat[:0]
        else:
            ...
            out_values = source.concat.index_select(0, gather_idx)
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        out_values = func(source.concat, values_dim, index_device, **kwargs)
...
def _packed_segment_index_select(source: NestedTensor, dim_adj: int, index: Tensor) -> NestedTensor:
    ...
    if count and offsets.numel() > 1 and not (_is_fake_tensor(source.concat) or _is_fake_tensor(index)):
    ...
    return _packed_with_shape(
        source,
        source.concat.index_select(0, rows),
        new_physical_shape,
...
def _packed_scatter_src(source: NestedTensor, src):
    ...
    if isinstance(src, NestedTensor):
        ...
        values = src.concat
        return values if values.device == source.concat.device else values.to(device=source.concat.device)
...
def _inner_segment_offsets(source: NestedTensor) -> Tensor:
    ...
    return offsets.to(device=source.concat.device, dtype=torch.long)
...
def _segmented_row_index(
    ...
    *,
    ...
    if row_batch_indices is None:
        starts, lengths = segment_row_bounds(offsets, source.concat.shape[0])
    else:
...
def _scatter_like(source, dim, index, src, apply_fn, op_name: str):
    ...
    if isinstance(index, NestedTensor) and src_values is not _NO_PACKED_SRC and source._has_same_structure(index):
        ...
        values_dim = _packed_static_dim(source, dim_adj)
        if values_dim is not None:
            return source._packed_like_unchecked(apply_fn(source.concat, values_dim, index_values, src_values))
        if _packed_inner_ragged_dim(source, dim_adj):
            packed_index = _segmented_row_index(source, index_values, dim, op_name)
            return source._packed_like_unchecked(apply_fn(source.concat, 0, packed_index, src_values))
...
def take(func, args, kwargs):
    ...
    if index.device != source.concat.device:
        index = index.to(device=source.concat.device)
    return func(source.concat.reshape(-1), index, **kwargs)
...
def _packed_new_dim_size(source: NestedTensor, new_values: Tensor, dim_adj: int, new_dim_size: int) -> NestedTensor:
    ...
    if (
        not _is_compiling()
        and type(new_values) is Tensor
        and dim_adj in source._static_dims
        and new_dim_size == source._max_physical_dims()[dim_adj]
...
def _packed_with_shape(
    ...
    *,
    ...
    if result._same_row_splits(source):
        source._share_offset_caches(result)
    if not _is_compiling():
        result._inherit_tensor_backed_dynamic_dims(source)
    return result._packed_like_unchecked(new_values, reuse_wrapper=True)
...
def _outer_size(
    ...
    if (
        ...
        and source._persistent_ragged_offsets() is not None
        and not _is_compiling()
        and not _is_fake_tensor(physical_shape)
    ):
        # A tensor-backed layout has an exact physical rank: a zero extent is a real dimension,
        # not a padding column, so it must survive rather than be trimmed off the maximum.
        element_shapes = tuple(tuple(int(size) for size in row) for row in physical_shape.tolist())
...
def _packed_batch_slice(source: NestedTensor, start: int, length: int, op_name: str) -> NestedTensor:
    ...
    return type(source)._from_packed(
        source.concat.narrow(0, row_start, row_count),
        offsets,
        physical_shape,
        permutation=source._permutation,
        ragged_dims=source._ragged_dims if source._ragged_dims_explicit else None,
...
def _packed_static_slice(source: NestedTensor, dim_adj: int, start: int, length: int) -> NestedTensor:
    ...
    return _packed_with_shape(
        source,
        source.concat.narrow(values_dim, start, length),
        shape,
...
def _packed_uniform_ragged_slice(source: NestedTensor, dim_adj: int, start: int, length: int) -> NestedTensor:
    ...
    return type(source)._from_packed(
        source.concat.index_select(0, rows.reshape(-1)),
        new_offsets,
        physical_shape,
        permutation=source._permutation,
        ragged_dims=source._ragged_dims if source._ragged_dims_explicit else None,
...
def _packed_multi_ragged_slice(source: NestedTensor, dim_adj: int, start: int, length: int) -> NestedTensor:
    r"""Keep a span of a collapsed ragged axis using only output-sized row indices.

    Ragged order determines the source stride, not logical dimension order.
    The preceding ragged coordinates form a prefix, and each selected position
    carries the complete ragged suffix and static feature block. Repacking such
    spans generally copies, as the existing sole-ragged slice does; batch and
    static-axis slices retain their separate view paths.
    """
    if _is_compiling() or any(
        _is_fake_tensor(tensor) for tensor in (source.concat, source._offsets, source._physical_shape)
    ):
        _compile_unsupported(
            "torch.narrow",
            "multi-ragged slicing requires concrete eager extents; dynamic output cardinality "
            "and ragged partitions are not implemented",
        )

    ragged = tuple(source._varying_dims)
    axis = ragged.index(dim_adj)
    shapes = source._element_shapes
    if shapes is None:
        shapes = tuple(tuple(int(size) for size in shape) for shape in source._physical_shape.tolist())
    extents, suffixes, starts, counts = [], [], [], []
    for position, shape in enumerate(shapes):
        extent = int(shape[dim_adj])
        ...
        prefix = math.prod(shape[dim] for dim in ragged[:axis])
        suffix = math.prod(shape[dim] for dim in ragged[axis + 1 :])  # noqa: E203
        extents.append(extent)
        suffixes.append(suffix)
        starts.append(resolved)
        counts.append(prefix * length * suffix)
    if not shapes:
        # An empty batch still carries logical non-batch extents. Respect their
        # bounds instead of accepting every span merely because no samples exist.
        extent = int(source._max_physical_dims()[dim_adj])
        resolved = start + extent if start < 0 else start
        if resolved < 0 or resolved + length > extent:
            raise RuntimeError(f"start ({start}) + length ({length}) exceeds dimension size ({extent}).")

    packed_sizes = tuple(counts)
    new_offsets = _offsets_from_packed_sizes(source, packed_sizes)
    total = sum(counts)
    ...
    if total == 0:
        ...
        counts_dev = torch.tensor(counts, device=device, dtype=torch.long)
        batch_index = torch.repeat_interleave(
            torch.arange(len(shapes), device=device, dtype=torch.long), counts_dev, output_size=total
        )
        local = torch.arange(total, device=device, dtype=torch.long) - new_offsets.to(device=device)[batch_index]
        suffix = torch.tensor(suffixes, device=device, dtype=torch.long)[batch_index]
        extent = torch.tensor(extents, device=device, dtype=torch.long)[batch_index]
        resolved = torch.tensor(starts, device=device, dtype=torch.long)[batch_index]
        block = length * suffix
        rows = (
            source._offsets.to(device=device, dtype=torch.long)[batch_index]
            + torch.div(local, block, rounding_mode="floor") * extent * suffix
            + resolved * suffix
    ...
    physical_shape = source._physical_shape.clone()
    physical_shape[:, dim_adj] = length
    element_shapes = tuple((*shape[:dim_adj], length, *shape[dim_adj + 1 :]) for shape in shapes)  # noqa: E203
    return _packed_with_shape(
        source,
        values.index_select(0, rows),
        physical_shape,
        source._logical_shape_from_components(replace_dims={dim_adj: length}),
        offsets=new_offsets,
        permutation=source._permutation,
        packed_sizes=packed_sizes,
        element_shapes=element_shapes,
...
def _graph_meta_equal(lhs: Tensor, rhs: Tensor, message: str) -> bool:
    r"""Compare metadata tensors under tracing, deferring the value comparison to a runtime assertion."""
    from .nested_tensor import NestedTensor

    return NestedTensor._meta_tensor_equal(lhs, rhs, message, runtime_assert=True)


def _same_batch_meta(lhs: NestedTensor, rhs: NestedTensor) -> bool:
    if len(lhs) != len(rhs) or lhs.batch_first != rhs.batch_first:
        return False
    if _is_compiling() and lhs._packed_sizes is None:
        return _graph_meta_equal(lhs._offsets, rhs._offsets, "NestedTensor packed matmul requires matching row splits")
...
def _packed_pair_indices(source: NestedTensor) -> tuple[Tensor, Tensor, tuple[int, ...]] | None:
    if source._packed_sizes is None and (_is_compiling() or _is_fake_tensor(source._offsets)):
        return None
    return _packed_pair_indices_from_sizes(source, _resolved_packed_sizes(source, "torch.matmul"))


def _packed_pair_indices_from_offsets(source: NestedTensor) -> tuple[Tensor, Tensor, Tensor]:
    r"""Build the same pair indices and per-sample lengths from row splits alone.

    The pair count is the sum of the squared lengths, which no tensor-backed layout reports as a
    Python integer, so it enters the graph as the unbacked size of the index tensors.
    """
    offsets = source.packed_offsets(dtype=torch.long)
    lengths = offsets[1:] - offsets[:-1]
    device = source.concat.device
    query, key = _complementary_square_indices(lengths, device, source.concat.shape[0])
    return query, key, lengths


def _packed_pair_indices_for_square(source: NestedTensor, square: NestedTensor) -> tuple[Tensor, Tensor]:
    r"""Pair indices of ``square`` against ``source`` rows when the pair count is already a graph size.

    ``square`` packs one ``length × length`` block per sample of ``source``, so its row count is
    the sum of the squared lengths. Reading that count off ``square.concat`` keeps the contraction
    free of a fresh data-dependent size: the count entered the graph when ``square`` was built.
    """
    device = source.concat.device
    offsets = source.packed_offsets(device=device, dtype=torch.long)
    lengths = offsets[1:] - offsets[:-1]
    pair_sizes = lengths * lengths
    total = square.concat.shape[0]
    sample = torch.repeat_interleave(
        torch.arange(lengths.numel(), device=device, dtype=torch.long),
        pair_sizes,
        output_size=total,
    )
    pair_starts = torch.cumsum(pair_sizes, 0) - pair_sizes
    local = torch.arange(total, device=device, dtype=torch.long) - pair_starts.index_select(0, sample)
    widths = lengths.index_select(0, sample)
    starts = offsets[:-1].index_select(0, sample)
    query = starts + torch.div(local, widths, rounding_mode="floor")
    key = starts + torch.remainder(local, widths)
    return query, key


def _packed_contraction_in_element_order(lhs: NestedTensor, rhs: NestedTensor, prefix_rank: int, rank: int) -> bool:
    r"""Whether both operands keep their static dims in element order, so packed tails contract as they stand."""
    lhs_static = (*range(prefix_rank), rank - 1)
    rhs_static = (*range(prefix_rank), prefix_rank)
    return lhs._static_dims == lhs_static and rhs._static_dims == rhs_static


def _jagged_outer_matmul_meta(lhs: NestedTensor, rhs: NestedTensor) -> tuple[int, tuple[int, ...] | None] | None:
    ...
    if lhs.concat.dim() != rank or rhs.concat.dim() != rank:
        return None
    if tuple(lhs.concat.shape[1:]) != tuple(rhs.concat.shape[1:]):
        return None
    if _is_compiling() and not _packed_contraction_in_element_order(lhs, rhs, prefix_rank, rank):
        return None
    if not _same_batch_meta(lhs, rhs):
        return None
    if lhs._element_shapes is not None and rhs._element_shapes is not None and lhs._packed_sizes is not None:
        if rhs._packed_sizes != lhs._packed_sizes:
            return None
    ...
    # Tensor-backed layouts carry no Python shape caches; compare the metadata tensors instead.
    if _is_compiling():
        # Both operands pack a single ragged dimension, so the matching row splits already state
        # that every matched extent agrees, and the contracted extents are the compared tails.
        return prefix_rank, None
    if _is_fake_tensor(lhs._offsets) or _is_fake_tensor(rhs._offsets):
        return None
    lhs_shape, rhs_shape = lhs._physical_shape, rhs._physical_shape
    if lhs_shape.shape != rhs_shape.shape or not torch.equal(lhs._offsets, rhs._offsets):
        return None
    if not torch.equal(lhs_shape[:, :prefix_rank], rhs_shape[:, :prefix_rank]):
        return None
    if not torch.equal(lhs_shape[:, prefix_rank], rhs_shape[:, rank - 1]):
        return None
    if not torch.equal(lhs_shape[:, rank - 1], rhs_shape[:, prefix_rank]):
        return None
    return prefix_rank, _resolved_packed_sizes(lhs, "torch.matmul")


def _jagged_square_contract_layout(lhs: NestedTensor, rhs: NestedTensor, prefix_rank: int, rank: int) -> bool:
    r"""Whether a square score operand contracts against a jagged operand, judged from tensor metadata alone."""
    if lhs._static_dims != tuple(range(prefix_rank)):
        return False
    if rhs._static_dims != (*range(prefix_rank), *range(prefix_rank + 1, rank)):
        return False
    lhs_shape, rhs_shape = lhs._physical_shape, rhs._physical_shape
    extent = rhs_shape[:, prefix_rank]
    message = "NestedTensor packed matmul requires scores squared over the contracted extent"
    if not _graph_meta_equal(lhs_shape[:, prefix_rank], extent, message):
        return False
    if not _graph_meta_equal(lhs_shape[:, prefix_rank + 1], extent, message):
        return False
    lengths = rhs.packed_offsets(dtype=torch.long)
    lengths = lengths[1:] - lengths[:-1]
    pair_offsets = torch.nn.functional.pad(lengths.square().cumsum(0), (1, 0))
    return _graph_meta_equal(
        lhs.packed_offsets(dtype=torch.long),
        pair_offsets,
        "NestedTensor packed matmul requires row-major square score storage",
    )


def _jagged_contract_matmul_meta(lhs: NestedTensor, rhs: NestedTensor) -> tuple[int, tuple[int, ...] | None] | None:
    ...
    if lhs._varying_dims != (prefix_rank, prefix_rank + 1) or rhs._varying_dims != (prefix_rank,):
        return None
    if lhs.concat.dim() != rank - 1 or rhs.concat.dim() != rank:
        return None
    if tuple(lhs.concat.shape[1:]) != tuple(rhs.concat.shape[1:-1]):
        return None
    if lhs._element_shapes is not None and rhs._element_shapes is not None and rhs._packed_sizes is not None:
        ...
        for lhs_shape, rhs_shape in zip(lhs._element_shapes, rhs._element_shapes):
            ...
            if lhs_shape[prefix_rank] != rhs_shape[prefix_rank] or lhs_shape[prefix_rank + 1] != rhs_shape[prefix_rank]:
                return None
        if not _is_fake_tensor(lhs._offsets) and not torch.equal(
            lhs._offsets, _offsets_from_packed_sizes(lhs, pair_sizes)
        ):
            return None
        return prefix_rank, tuple(int(size) for size in rhs._packed_sizes)
    # Tensor-backed layouts carry no Python shape caches; compare the metadata tensors instead.
    if _is_compiling():
        if not _jagged_square_contract_layout(lhs, rhs, prefix_rank, rank):
            return None
        return prefix_rank, None
    if _is_fake_tensor(lhs._offsets) or _is_fake_tensor(rhs._offsets):
        return None
    ...
    if not torch.equal(lhs_shape[:, prefix_rank], rhs_shape[:, prefix_rank]):
        return None
    if not torch.equal(lhs_shape[:, prefix_rank + 1], rhs_shape[:, prefix_rank]):
        return None
    sizes = _resolved_packed_sizes(rhs, "torch.matmul")
    pair_sizes = tuple(size * size for size in sizes)
    if not torch.equal(lhs._offsets, _offsets_from_packed_sizes(lhs, pair_sizes)):
        return None
    return prefix_rank, sizes
...
def _packed_jagged_outer_matmul(lhs: NestedTensor, rhs: NestedTensor) -> NestedTensor | None:
    ...
    if pair_indices is None:
        return _packed_jagged_outer_matmul_from_offsets(lhs, rhs, prefix_rank)
    query, key, pair_sizes = pair_indices
    values = (lhs.concat[query] * rhs.concat[key]).sum(-1)
...
def _packed_jagged_outer_matmul_from_offsets(
    lhs: NestedTensor,
    rhs: NestedTensor,
    prefix_rank: int,
) -> NestedTensor:
    r"""Contract matched packed rows into square elements when only row splits describe the sizes."""
    query, key, lengths = _packed_pair_indices_from_offsets(lhs)
    values = (lhs.concat[query] * rhs.concat[key]).sum(-1)
    result = _restore_complementary_square_prefix(lhs.packed_with_square_lengths(values, lengths), prefix_rank)
...
def _packed_jagged_contract_matmul(lhs: NestedTensor, rhs: NestedTensor) -> NestedTensor | None:
    ...
    if pair_indices is None:
        query, key = _packed_pair_indices_for_square(rhs, lhs)
    else:
        query, key, _ = pair_indices
    values = torch.zeros_like(rhs.concat).index_add(
        0, query, lhs.concat.unsqueeze(-1) * rhs.concat.index_select(0, key)
    )
...
def _packed_square_softmax(
    ...
    *,
    ...
    if dim_adj != prefix_rank + 1 or source._varying_dims != (prefix_rank, prefix_rank + 1):
        return None
    if source.concat.dim() != rank - 1:
        return None
    if source._element_shapes is not None and source._packed_sizes is not None:
        sizes = []
        ...
        # Tensor-backed layouts carry no Python shape caches; read the square extents off the shape tensor.
        if _is_compiling() or _is_fake_tensor(source._offsets):
            return None
        shape_tensor = source._physical_shape
        if not torch.equal(shape_tensor[:, prefix_rank], shape_tensor[:, prefix_rank + 1]):
            return None
        sizes_tuple = tuple(int(size) for size in shape_tensor[:, prefix_rank].tolist())
        pair_sizes = tuple(size * size for size in sizes_tuple)
        if not torch.equal(source._offsets, _offsets_from_packed_sizes(source, pair_sizes)):
            return None
    query, _, _ = _packed_pair_indices_from_sizes(source, sizes_tuple)
    total = sum(sizes_tuple)
    source_values = source.concat
    values = (
        source_values.float()
...
def _matmul_has_packed_path(lhs, rhs) -> bool:
    ...
    if isinstance(lhs, NestedTensor):
        if isinstance(rhs, NestedTensor):
            return (
                (lhs._has_same_structure(rhs) and lhs.concat.dim() > 2 and rhs.concat.dim() > 2)
                or (_packed_jagged_matmul_kind(lhs, rhs) is not None)
                or (
                    lhs.concat.dim() == rhs.concat.dim() == 4
                    and _complementary_singleton_square_operands(lhs, rhs) is not None
                )
            )
        return isinstance(rhs, Tensor) and lhs.concat.dim() >= 2 and rhs.dim() <= 2
    if isinstance(rhs, NestedTensor):
        if isinstance(lhs, Tensor) and lhs.dim() <= 2 and rhs.concat.dim() > 2:
            return True
        if not isinstance(lhs, Tensor) or rhs.concat.dim() != 2:
            return False
        if lhs.dim() == 2 and 0 not in rhs._varying_dims:
            return True
        return lhs.dim() >= 2 and _uniform_matrix_extents(rhs) is not None
    return False


def _uniform_matrix_extents(source: NestedTensor) -> tuple[int, int] | None:
    r"""Return ``(rows, cols)`` when every 2-D element has the same shape, so the batch views as ``[B, rows, cols]``."""
    if source._physical_shape.size(1) != 2 or 1 in source._varying_dims:
        return None
    rows = _uniform_physical_extent(source._physical_shape, 0)
    cols = _uniform_physical_extent(source._physical_shape, 1)
...
def _check_nonempty_extrema_segments(source: NestedTensor, func) -> None:
    ...
    if source._packed_sizes is not None:
        has_empty = any(int(size) == 0 for size in source._packed_sizes)
    elif _is_compiling() or _is_fake_tensor(source._offsets):
        has_empty = False
    else:
        has_empty = bool((source._offsets[1:] == source._offsets[:-1]).any())
    if has_empty:
...
def _segment_count_nonzero_ragged_dim(source: NestedTensor, dim_adj: int) -> Tensor | None:
    if not _has_single_packed_ragged_dim(source, dim_adj):
        return None
    values = source.concat.ne(0).to(dtype=torch.long)
    offsets = source.ragged_level_offsets(0, device=values.device, dtype=torch.long)
...
def _segment_vector_norm_ragged_dim(
    ...
    *,
    ...
    if ord_float is None or not source.concat.dtype.is_floating_point or source.concat.dtype.is_complex:
        return None

    values = source.concat if dtype is None else source.concat.to(dtype=dtype)
    offsets = source.ragged_level_offsets(0, device=values.device, dtype=torch.long)
    lengths = offsets[1:] - offsets[:-1]
...
def _boolean_scatter_carrier(values: Tensor) -> Tensor:
    r"""Represent logical values with a dtype supported by CUDA ``scatter_reduce``."""
...
def _packed_boolean_batch_reduction(func, source: NestedTensor, keepdim: bool) -> Tensor | None:
    ...
    values = _boolean_values(source.concat)
    ...
    result_dtype = rows.dtype
    rows = _boolean_scatter_carrier(rows)
...
def _packed_ragged_group_counts(source: NestedTensor, dim_adj: int) -> Tensor:
    r"""Return how many groups every sample owns once ``dim_adj`` leaves its packed coordinates."""
    ...
    return source._physical_shape[:, list(remaining_ragged)].prod(dim=1)


def _packed_ragged_group_indices(
    source: NestedTensor,
    dim_adj: int,
    group_counts: Tensor,
    *,
    ...
    r"""Return the group of every packed row, that is its packed coordinate with ``dim_adj`` dropped."""
    shape_device = source._physical_shape.to(device=device, dtype=torch.long)
    counts = group_counts.to(device=device, dtype=torch.long)
    group_offsets = torch.cat((counts.new_zeros(1), counts.cumsum(0)))
    ...
    return group_offsets.index_select(0, batch_idx) + local_group
...
def _packed_ragged_reduction_groups(
    ...
    *,
    ...
    remaining_ragged = tuple(dim for dim in source._varying_dims if dim != dim_adj)
    physical_group_counts = _packed_ragged_group_counts(source, dim_adj)
    groups = _packed_ragged_group_indices(source, dim_adj, physical_group_counts, device=device)
...
def _packed_boolean_ragged_reduction(
    ...
    values = _boolean_scatter_carrier(values)
...
def _numeric_reduction_values(values: Tensor, dtype: torch.dtype, *, mean: bool) -> Tensor:
    """Prepare reduction inputs without rounding a CPU mean before accumulation."""
    low_precision = dtype in (torch.float16, torch.bfloat16)
    if mean and values.device.type == "cpu" and low_precision:
        return values.float()
...
def _packed_numeric_ragged_reduction(
    ...
    if func in (aten.sum.dim_IntList, aten.mean.dim):
        ...
        add_values = _numeric_reduction_values(values, sample.dtype, mean=func is aten.mean.dim)
        output = add_values.new_zeros((output_size, *sample.shape))
        ...
        output = output.to(dtype=sample.dtype)
    else:
        if values.dtype.is_complex:
            return None
...
def _segment_reduce_ragged_dim(
    ...
    if func is aten.sum.dim_IntList:
        ...
        sample = func(values[:0], [0], False, **kwargs)
        segment_values = _numeric_reduction_values(values, sample.dtype, mean=True)
        if (
            segment_reduce is not None
            and segment_values.dtype.is_floating_point
            and not segment_values.dtype.is_complex
        ):
            out = segment_reduce(segment_values, reduce="mean", lengths=lengths).to(dtype=sample.dtype)
            return _format_permuted_segment_reduction(source, out, dim_adj, keepdim)
        out = segment_values.new_zeros((batch_size, *sample.shape))
        batch_idx = source.packed_batch_indices(device=values.device)
        out = out.index_add(0, batch_idx, segment_values)
        denominator = lengths.reshape(batch_size, *([1] * (out.dim() - 1)))
        out = (out / denominator).to(dtype=sample.dtype)
...
def _sdpa_pack_native(nt: NestedTensor) -> tuple[Tensor, Tensor, int]:
    ...
    if nt._element_shapes is not None and all(
        ...
        # Native elements have shape (heads, sequence, features). The sequence
        # maximum is already part of the logical shape in both batch orders.
        # Preserve its SymInt instead of specializing a tensor-data reduction.
        max_seqlen = nt._logical_shape[-2]
    return nt.concat.contiguous(), cumulative, max_seqlen
...
def _sdpa_via_native_flash(
    ...
    *,
    ...
    if _same_ragged_offsets(query, key):
        k_values = key.concat.contiguous()
...
def addmm(func, args, kwargs):
    ...
    if isinstance(mat1, NestedTensor) and not isinstance(mat2, NestedTensor) and mat1.concat.dim() == 2:
        new_values = func(bias, mat1.concat, mat2, **kwargs)
    ...
    if (
        ...
        if 0 not in mat2._varying_dims:
            ...
            if packed_bias is not None:
                new_values = func(packed_bias, mat2.concat, mat1.transpose(0, 1), **kwargs)
                return _packed_new_dim_size(mat2, new_values, 0, int(mat1.shape[0]))
        extents = _uniform_matrix_extents(mat2)
        if extents is not None:
            rhs_batched = mat2.concat.view(len(mat2), *extents)
            bias_batched = bias.expand(len(mat2), *bias.shape) if isinstance(bias, Tensor) and bias.dim() >= 1 else bias
            out = torch.baddbmm(
                bias_batched,
            ...
            return _from_uniform_batched_output(mat2, out)
...
def baddbmm(func, args, kwargs):
    ...
    if (
        ...
        and batch1.dim() == 3
        and batch2.concat.dim() == 3
        and (
            (0 not in batch2._varying_dims and 1 not in batch2._varying_dims)
            or (
                batch2._physical_shape.size(1) > 1
                and 1 not in batch2._varying_dims
                and _uniform_physical_extent(batch2._physical_shape, 0) is not None
            )
        )
    ):
        ...
        new_values = func(packed_bias, batch2.concat.permute(1, 0, 2), batch1.transpose(1, 2), **kwargs).permute(
            1, 0, 2
        )
...
def bmm(func, args, kwargs):
    ...
    if isinstance(mat1, NestedTensor) and isinstance(mat2, NestedTensor) and mat1._has_same_structure(mat2):
        new_values = func(mat1.concat, mat2.concat, **kwargs)
        return _packed_new_last_dim(mat1, new_values, mat2.concat.shape[-1])
    if (
        isinstance(mat1, Tensor)
        and not isinstance(mat1, NestedTensor)
        and isinstance(mat2, NestedTensor)
        and mat1.dim() == 3
        and mat2.concat.dim() == 3
        and (
            (0 not in mat2._varying_dims and 1 not in mat2._varying_dims)
            or (
                mat2._physical_shape.size(1) > 1
                and 1 not in mat2._varying_dims
                and _uniform_physical_extent(mat2._physical_shape, 0) is not None
            )
        )
    ):
        new_values = torch.bmm(mat2.concat.permute(1, 0, 2), mat1.transpose(1, 2)).permute(1, 0, 2)
...
def diagonal(func, args, kwargs):
    ...
    out_values = func(source.concat, offset, dim1_adj, dim2_adj, **kwargs)
...
def linalg_check_errors(func, args, kwargs):
    ...
    if isinstance(info, NestedTensor):
        func(info.concat, *args[1:], **kwargs)
        return None
    return func(*args, **kwargs)
...
def linalg_eigh(func, args, kwargs):
    ...
    eigvals_values, eigvecs_values = func(source.concat, *args[1:], **kwargs)
...
def linalg_qr(func, args, kwargs):
    ...
    q_values, r_values = func(source.concat, *args[1:], **kwargs)
...
def linalg_solve(func, args, kwargs):
    ...
    if isinstance(mat_a, NestedTensor):
        if isinstance(mat_b, NestedTensor):
            ...
            if mat_a._has_same_structure(mat_b) and mat_a.concat.dim() > 2 and mat_b.concat.dim() > 1:
                return _packed_with_tail_from_values(mat_a, func(mat_a.concat, mat_b.concat, *args[2:], **kwargs))
            return per_element_fallback(func, args, kwargs)
        if isinstance(mat_b, Tensor) and mat_a.concat.dim() > 2:
            return _packed_with_tail_from_values(mat_a, func(mat_a.concat, mat_b, *args[2:], **kwargs))
        return per_element_fallback(func, args, kwargs)

    if isinstance(mat_b, NestedTensor):
        if isinstance(mat_a, Tensor) and mat_a.dim() == 2 and mat_b.concat.dim() > 1:
            return _packed_with_tail_from_values(mat_b, func(mat_a, mat_b.concat, *args[2:], **kwargs))
        return per_element_fallback(func, args, kwargs)
...
def linalg_solve_ex(func, args, kwargs):
    ...
    if isinstance(mat_a, NestedTensor):
        if isinstance(mat_b, NestedTensor):
            ...
            if mat_a._has_same_structure(mat_b) and mat_a.concat.dim() > 2 and mat_b.concat.dim() > 1:
                result, lu, pivots, info = func(mat_a.concat, mat_b.concat, *args[2:], **kwargs)
                return (
            ...
            return per_element_fallback(func, args, kwargs)
        if isinstance(mat_b, Tensor) and mat_a.concat.dim() > 2:
            result, lu, pivots, info = func(mat_a.concat, mat_b, *args[2:], **kwargs)
            return (
                _packed_with_tail_from_values(mat_a, result),
                mat_a._packed_like_unchecked(lu),
                _packed_with_tail_from_values(mat_a, pivots),
...
def linalg_svd(func, args, kwargs):
    ...
    u_values, s_values, vh_values = func(source.concat, *args[1:], **kwargs)
    ...
    if isinstance(lhs, NestedTensor):
        if isinstance(rhs, NestedTensor):
            ...
            if jagged is not None:
                return jagged
            if lhs.concat.dim() == rhs.concat.dim() == 4:
                square = _binary_complementary_singleton_square(lhs, rhs, torch.bmm, (), kwargs)
                if square is not None:
                    return square
            if lhs._has_same_structure(rhs) and lhs.concat.dim() > 2 and rhs.concat.dim() > 2:
                return _packed_with_tail_from_values(lhs, func(lhs.concat, rhs.concat, **kwargs))
            return per_element_fallback(func, args, kwargs)
        if isinstance(rhs, Tensor) and lhs.concat.dim() >= 2 and rhs.dim() <= 2:
            return _packed_with_tail_from_values(lhs, func(lhs.concat, rhs, **kwargs))
        return per_element_fallback(func, args, kwargs)

    if isinstance(rhs, NestedTensor):
        if isinstance(lhs, Tensor) and lhs.dim() == 2 and rhs.concat.dim() == 2:
            if 0 not in rhs._varying_dims:
                new_values = torch.mm(rhs.concat, lhs.transpose(0, 1))
                return _packed_new_dim_size(rhs, new_values, 0, int(lhs.shape[0]))
            extents = _uniform_matrix_extents(rhs)
            if extents is not None:
                rhs_batched = rhs.concat.view(len(rhs), *extents)
                return _from_uniform_batched_output(rhs, func(lhs, rhs_batched, **kwargs))
        if isinstance(lhs, Tensor) and lhs.dim() > 2 and rhs.concat.dim() == 2:
            extents = _uniform_matrix_extents(rhs)
            if extents is not None:
                rhs_batched = rhs.concat.view(len(rhs), *extents)
                out = func(lhs.unsqueeze(0), rhs_batched.unsqueeze(1), **kwargs)
                return _from_uniform_batched_output(rhs, out.reshape(len(rhs), *lhs.shape[:-1], extents[1]))
        if isinstance(lhs, Tensor) and lhs.dim() <= 2 and rhs.concat.dim() > 2:
            return _packed_with_tail_from_values(rhs, func(lhs, rhs.concat, **kwargs))
        return per_element_fallback(func, args, kwargs)
...
def matrix_last2_to_scalar(func, args, kwargs):
    ...
    if source.concat.dim() <= 2:
        ...
        padded = source.concat.new_zeros((batch, max_rows, max_cols))
        if source.concat.numel() > 0:
            padded[source._packed_dense_index(device=device)] = source.concat
        ...
        eye = torch.eye(max_rows, max_cols, dtype=source.concat.dtype, device=device).expand(batch, -1, -1)
        values = func(torch.where(inside, padded, eye), *args[1:], **kwargs)
        return source._from_scalar_result_values(values)
    out_values = func(source.concat, *args[1:], **kwargs)
...
def mm(func, args, kwargs):
    ...
    mat1, mat2 = args[0], args[1]
    if isinstance(mat1, NestedTensor) and not isinstance(mat2, NestedTensor) and mat1.concat.dim() == 2:
        new_values = func(mat1.concat, mat2, **kwargs)
        return _packed_new_last_dim(mat1, new_values, mat2.shape[1])
    if (
        not isinstance(mat1, NestedTensor)
        and isinstance(mat2, NestedTensor)
        ...
        if 0 not in mat2._varying_dims:
            new_values = func(mat2.concat, mat1.transpose(0, 1), **kwargs)
            return _packed_new_dim_size(mat2, new_values, 0, int(mat1.shape[0]))
        extents = _uniform_matrix_extents(mat2)
        if extents is not None:
            rhs_batched = mat2.concat.view(len(mat2), *extents)
            return _from_uniform_batched_output(mat2, torch.matmul(mat1, rhs_batched))
...
def native_layer_norm(func, args, kwargs):
    ...
    source = args[0]
    output, mean, rstd = func(source.concat, *args[1:], **kwargs)
...
def native_layer_norm_backward(func, args, kwargs):
    ...
    if len(sources) == 2 and not sources[0]._has_same_layout(sources[1]):
        return per_element_fallback(func, args, kwargs)
    ref = sources[0]
    g = grad_out.concat if isinstance(grad_out, NestedTensor) else grad_out
    i = input_.concat if isinstance(input_, NestedTensor) else input_
...
def flatten(func, args, kwargs):
    ...
    if start <= batch_dim <= end:
        if source._ragged_rank >= 2 and start == 0 and end == 1:
            element_shapes = source._element_shapes
            if element_shapes is None:
                element_shapes = tuple(tuple(int(size) for size in shape) for shape in source._original_shapes())
            ...
            if source._permutation[:1] == (0,):
                ...
                if source.batch_first:
                    ...
                    starts_t = torch.tensor(starts, dtype=torch.long, device=source.concat.device)
                    lengths_t = torch.tensor(packed_sizes, dtype=torch.long, device=source.concat.device)
                    row_id = torch.repeat_interleave(
                        torch.arange(len(row_pairs), device=source.concat.device),
                        lengths_t,
                    )
                    prefix = lengths_t.cumsum(0)
                    local = (
                        torch.arange(int(prefix[-1]), device=source.concat.device) - prefix[row_id] + lengths_t[row_id]
                    )
                    values = source.concat.index_select(0, starts_t[row_id] + local)
    ...
    out_values = func(source.concat, values_start, values_end, **kwargs)
...
def permute(func, args, kwargs):
    ...
    out_values = func(source.concat, list(tensor_dims), **kwargs)
    out_shape = source._physical_shape[:, tensor_dims]
    out_logical = source._logical_shape_from_physical_dims(
        tuple(source._max_physical_dims()[dim] for dim in tensor_dims)
    )
...
def squeeze_default(func, args, kwargs):
    ...
    if rank == 0:
        return source._packed_like_unchecked(source.concat)
    ...
    if source._physical_shape.size(0) > 0 and bool(torch.any(source._physical_shape[:, 0] == 1)):
        ...
        if element_shapes is None:
            element_shapes = tuple(tuple(int(size) for size in shape) for shape in source._original_shapes())
        first_shape = element_shapes[0] if element_shapes else ()
        keep_dims = tuple(dim for dim, size in enumerate(first_shape) if int(size) != 1)
        if all(tuple(dim for dim, size in enumerate(shape) if int(size) != 1) == keep_dims for shape in element_shapes):
    ...
    out_values = func(source.concat, **kwargs)
...
def squeeze_dim(func, args, kwargs):
    ...
    values_dim = _physical_to_values_dim(source, dim_adj)
    if values_dim is not None:
        if source.concat.shape[values_dim] != 1:
            return source
        return _packed_without_dim(source, dim_adj, func(source.concat, values_dim, **kwargs))
...
def narrow_nested(source: NestedTensor, dim: int, start, length: int) -> NestedTensor:
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        extent = int(source.concat.shape[values_dim])
        resolved = start + extent if start < 0 else start
        if resolved < 0 or resolved + length > extent:
            raise RuntimeError(f"start ({start}) + length ({length}) exceeds dimension size ({extent}).")
    ...
    if source._ragged_rank > 1 and dim_adj in source._varying_dims:
        rank = int(source._physical_shape.size(1))
        if source._element_shapes is None or all(len(shape) == rank for shape in source._element_shapes):
            return _packed_multi_ragged_slice(source, dim_adj, start, length)
...
def transpose(func, args, kwargs):
    ...
    out_values = func(source.concat, packed_dim0, packed_dim1, **kwargs)
...
def unflatten(func, args, kwargs):
    ...
    out_values = func(source.concat, values_dim, sizes, **kwargs)
    inserted_rank = out_values.dim() - source.concat.dim() + 1
...
@NestedTensorAtenRegistry.implement(aten.unsqueeze.default, compile_safe=True)
...
def _view_element_target(source: NestedTensor, target: Sequence[int], max_sizes: Sequence[int]) -> list[int]:
    r"""Strip the batch entry from a view-like target, mirroring :meth:`NestedTensor._view_shapes`."""
    batch_dim = _get_batch_dim(source)
    batch_size = len(source)
    include_batch = False
    if len(target) > batch_dim:
        if target[batch_dim] == batch_size and len(target) != source.dim():
            include_batch = True
        elif target[batch_dim] in (-1, batch_size) and len(target) == source.dim():
            non_batch = [dim for dim in range(len(target)) if dim != batch_dim]
            include_batch = any(
                index < len(max_sizes) and (target[dim] == -1 or target[dim] == max_sizes[index])
                for index, dim in enumerate(non_batch)
            )
    element_target = list(target)
    if include_batch:
        if element_target[batch_dim] == -1:
            element_target[batch_dim] = batch_size
        if element_target[batch_dim] != batch_size:
            raise ValueError(f"Batch dimension mismatch: expected {batch_size} but got {element_target[batch_dim]}")
        element_target.pop(batch_dim)
    return element_target


def _view_target_columns(element_target: Sequence[int], max_sizes: Sequence[int]) -> list[int | None]:
    r"""Map each element-target entry to the source element dim it substitutes, or ``None`` for a constant."""
    columns: list[int | None] = []
    available = list(range(len(max_sizes)))
    for index, size in enumerate(element_target):
        if index >= len(max_sizes) or size == -1:
            columns.append(None)
            continue
        if size == max_sizes[index]:
            columns.append(index)
            if index in available:
                available.remove(index)
            continue
        candidates = [dim for dim in available if max_sizes[dim] == size]
        if len(candidates) == 1:
...
def _tensor_backed_view_like(func, source: NestedTensor, target: tuple[int, ...], kwargs) -> NestedTensor:
    r"""Apply a view-like reshape from metadata tensors alone, for layouts without per-element shapes.

    Only the leading output dimension may differ between elements: every other entry is either a
    constant of the target or a static source dim, so the packed rows stay a single ``view`` of
    ``concat``. Anything else would need per-element shapes and is rejected explicitly.
    """
    max_sizes = list(source._max_physical_dims())
    element_target = _view_element_target(source, target, max_sizes)
    rank = len(element_target)
    if rank == 0 or source._ragged_rank != 1 or not _is_packed_identity(source):
        _compile_unsupported(
            "NestedTensor view/reshape",
            "tensor-backed rebuilds need one leading ragged dimension packed in element order",
        )

    ragged_dim = source._ragged_dims[0]
    columns = _view_target_columns(element_target, max_sizes)
    if any(size == -1 for size in element_target[1:]) or any(column == ragged_dim for column in columns[1:]):
        _compile_unsupported(
            "NestedTensor view/reshape",
            "only the leading target dimension may vary across elements",
        )

    tail_numel = 1
    for size in element_target[1:]:
        tail_numel *= size
    if tail_numel == 0:
        _compile_unsupported("NestedTensor view/reshape", "a zero-sized target tail cannot size the packed rows")

    # The single ragged dim scales every element numel, so its maximum attains the maximum numel.
    static_numel = 1
    for dim in source._static_dims:
        static_numel *= max_sizes[dim]

    ones = torch.ones_like(source._physical_shape[:, :1])
    keeps_rows = columns[0] == ragged_dim or (element_target[0] == -1 and tail_numel == static_numel)
    if keeps_rows:
        lengths = source._physical_shape[:, ragged_dim : ragged_dim + 1]
        max_length = max_sizes[ragged_dim]
    elif element_target[0] == -1:
        lengths = source._physical_shape.prod(dim=1, keepdim=True) // tail_numel
        max_length = max_sizes[ragged_dim] * static_numel // tail_numel
    else:
        lengths = ones * element_target[0]
        max_length = element_target[0]

    out_physical_shape = torch.cat((lengths, *(ones * size for size in element_target[1:])), dim=1)
    out_values = func(source.concat, [-1, *element_target[1:]], **kwargs)
    if keeps_rows:
        out_offsets = source._offsets
    else:
        packed_rows = lengths.reshape(-1).to(device=source._offsets.device, dtype=source._offsets.dtype)
        out_offsets = torch.cat((source._offsets.new_zeros((1,)), torch.cumsum(packed_rows, dim=0)))

    return _packed_with_shape(
        source,
        out_values,
        out_physical_shape,
        source._logical_shape_from_physical_dims([max_length, *element_target[1:]]),
        offsets=out_offsets,
        permutation=tuple(range(rank)),
    )
...
def view_like(func, args, kwargs):
    ...
    if source._element_shapes is None and (_is_compiling() or _is_fake_tensor(source._physical_shape)):
        return _tensor_backed_view_like(func, source, target, kwargs)
    ...
    def rebuild_per_element():
        ...
        # Only permuted packings reach this path, so the elements are permuted views of packed
        # rows and a copying reshape is the only way to produce the requested element shapes.
        outputs = [torch.reshape(t, list(s)) for t, s in zip(source._unpack(), view_shapes)]
        # A view chooses new element shapes; their layout is inferred from the results.
        return type(source)(outputs, **{**source._meta(), "ragged_dims": None})
    ...
    if not _is_packed_identity(source):
        # The packed view reads rows in element order; a permuted packing has to go per element.
        return rebuild_per_element()
    ...
    out_values = func(source.concat, packed_shape, **kwargs)
    ...
    if packed_sizes == source._packed_sizes or (
        not _is_fake_tensor(source._offsets)
        and source._offsets.numel() == lengths_tensor.numel() + 1
        and torch.equal(lengths_tensor, source._offsets[1:] - source._offsets[:-1])
    ):
        out_offsets = source._offsets
    else:
        out_offsets = torch.empty(
...
def _binary_unwrap_handler(func, args, kwargs):
    ...
    if len(sources) == 2 and not sources[0]._has_same_layout(sources[1]):
        return per_element_fallback(func, args, kwargs)
    ref = sources[0]
    va = a.concat if isinstance(a, NestedTensor) else a
    vb = b.concat if isinstance(b, NestedTensor) else b
...
def _packed_varying_softmax_group_indices(
    ...
    *,
    ...
    element_shapes = source._element_shapes
    if element_shapes is None:
        # Tensor-backed layouts carry no Python shape caches: the rows of one softmax group share every
        # packed coordinate but ``target_dim``, so the metadata tensors alone describe the grouping. An
        # element with an empty target extent owns no packed row and must not claim group ids either.
        if _is_fake_tensor(source._physical_shape) and not _is_compiling():
            return None
        counts = _packed_ragged_group_counts(source, target_dim) * (source._physical_shape[:, target_dim] > 0)
        group_idx = _packed_ragged_group_indices(source, target_dim, counts, device=device)
        if target_dim == source._varying_dims[-1]:
            # Reducing the innermost ragged level groups the packed rows by the level above it.
            return group_idx, source._ragged_level_offsets(-1).numel() - 1
        # The group total is data dependent for any other level; the packed row count bounds it, so the
        # segment buffers keep a symbolic extent instead of binding an unbacked one under compile.
        group_count = source.concat.shape[0] if _is_compiling() else int(counts.sum())
        return group_idx, group_count

    batch_idx, local_idx = source._packed_batch_local_indices(device=device, dtype=torch.long)
    varying_coords = source._packed_varying_coords(batch_idx, local_idx, device=device, dtype=torch.long)
    excluded = tuple(
        (physical_dim, coord)
        for physical_dim, coord in zip(source._varying_dims, varying_coords)
...
def _packed_varying_softmax(
    ...
    *,
    ...
    if not _is_compiling() and values.shape[0] == 0:
        return source._packed_like_unchecked(values if half_to_float else source_values)

    groups = _packed_varying_softmax_group_indices(source, target_dim, device=values.device)
    if groups is None:
        return None
...
def _softmax_handler(func, args, kwargs):
    ...
    return source._packed_like_unchecked(func(source.concat, values_dim, *args[2:], **kwargs))
...
def _flip_compile_safe(args: tuple, kwargs: dict[str, object]) -> bool:
    ...
    if not varying_dims:
        return source.concat.dim() > 1 and all(dim in source._static_dims for dim in dims_adj)
    # A single packed ragged dim flips through tensor-only index arithmetic on the offsets.
    return (
        len(set(varying_dims)) == 1
        and _has_single_packed_ragged_dim(source, varying_dims[0])
        and all(dim in source._static_dims or dim == varying_dims[0] for dim in dims_adj)
    )
...
def argsort(func, args, kwargs):
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        return source._packed_like_unchecked(_call_argsort(source.concat, values_dim))
    if _has_single_packed_ragged_dim(source, dim_adj):
...
def cumulative(func, args, kwargs):
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        return source._packed_like_unchecked(func(source.concat, values_dim, *extra_args, **kwargs))
    if _has_single_packed_ragged_dim(source, dim_adj):
        ...
        result_dtype = source.concat.dtype if dtype is None else dtype
        scanned = segmented_scan(source.concat.to(result_dtype), source.packed_batch_indices(), torch.logaddexp)
...
def cumulative_pair(func, args, kwargs):
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        vals, idxs = func(source.concat, values_dim, **kwargs)
    ...
    if _has_single_packed_ragged_dim(source, dim_adj):
        ...
        values, indices = segmented_arg_scan(
            source.concat,
            source.packed_batch_indices(),
            source.packed_local_indices(device=source.concat.device),
            largest=largest,
        )
...
def flip(func, args, kwargs):
    ...
    if len(varying_dims) == 0 and all(dim in source._static_dims for dim in dims_adj):
        ...
        return source._packed_like_unchecked(func(source.concat, packed_dims, **kwargs))
...
def roll(func, args, kwargs):
    ...
    dims_adj = tuple(_translate_dim(source, dim) for dim in dims)
    if source.concat.dim() > 1 and all(dim > 0 for dim in dims_adj):
        return source._packed_like_unchecked(func(source.concat, shifts, list(dims_adj), **kwargs))
    if _is_compiling():
...
def rot90(func, args, kwargs):
    ...
    if source.concat.dim() > 1 and all(dim > 0 for dim in dims_adj):
        out_values = func(source.concat, k, list(dims_adj), **kwargs)
        if k_mod % 2 == 0:
            return source._packed_like_unchecked(out_values)
        out_shape = source._physical_shape.clone()
...
def searchsorted_tensor(func, args, kwargs):
    ...
    if sorted_is_nt and values_is_nt:
        ...
        offsets_match = (
            sorted_sequence.concat.dim() >= 2
            and values.concat.dim() >= 2
...
def sort(func, args, kwargs):
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        vals, idxs = _call_sort(source.concat, values_dim)
        return source._packed_like_unchecked(vals), source._packed_like_unchecked(idxs)
    if _has_single_packed_ragged_dim(source, dim_adj):
        ...
        return source._packed_like_unchecked(torch.gather(source.concat, 0, perm)), source._packed_like_unchecked(local)
    if stable_overload or stable is not None:
        return per_element_fallback(
...
def topk(func, args, kwargs):
    ...
    values_dim = _packed_static_dim(source, dim_adj)
    if values_dim is not None:
        vals, idxs = func(source.concat, k, values_dim, largest, sorted_output, **kwargs)
        return (
    ...
    if _has_single_packed_ragged_dim(source, dim_adj):
        ...
        lengths = offsets[1:] - offsets[:-1]
        k_value = int(k)
        if lengths.numel() > 0 and not (_is_compiling() or _is_fake_tensor(source.concat)):
            shortest = int(lengths.min())
            if k_value > shortest:
                raise RuntimeError(f"selected index k out of range: k={k_value} exceeds shortest segment {shortest}")

        perm, local = segmented_sort_perm(
            source.concat, source._offsets, source.packed_batch_indices(), descending=largest
        )
        ...
        return (
            _packed_new_ragged_size(
                source, torch.gather(source.concat, 0, perm).index_select(0, keep), dim_adj, k_value
            ),
...
def alias(func, args, kwargs):
    ...
    source = args[0]
    return source._packed_like_unchecked(func(source.concat, **kwargs))
...
def constant_pad_nd(func, args, kwargs):
    ...
    if source.concat.dim() <= 1 or padded_dims >= source.concat.dim():
        return per_element_fallback(func, args, kwargs)
...
def _constant_pad_packed_variable_last_dim(source: NestedTensor, pad: tuple[int, int], value) -> NestedTensor | None:
    ...
    old_total = source.concat.size(0)
    new_total = old_total + len(source) * pad_width
    output_values = source.concat.new_full((new_total, *source.concat.shape[1:]), value)
    batch_indices = source.packed_batch_indices(device=source.concat.device)
    source_indices = torch.arange(old_total, device=source.concat.device)
    destination_indices = source_indices + batch_indices * pad_width + left
    output_values.index_copy_(0, destination_indices, source.concat)
...
@NestedTensorAtenRegistry.implement(aten.detach.default, compile_safe=True)
def detach(func, args, kwargs):
    ...
    source = args[0]
    if (
        type(source) is NestedTensor
        and type(source._packed_values) is Tensor
        and not _is_compiling()
        and _get_current_dispatch_mode() is None
        and not (_is_fake_tensor(source._offsets) or _is_fake_tensor(source._physical_shape))
    ):
        # Integer layout metadata never participates in autograd. Its previous
        # detach() views already shared storage/version counters; retain the
        # same metadata objects instead of normalizing a fresh layout. Keep the
        # actual numerical detach and the outer Tensor detach dispatch intact,
        # including PyTorch's wrapper version sharing and checkpoint hooks.
        return source._packed_like_unchecked_raw(source.concat.detach())
    ragged_offsets = source._persistent_ragged_offsets()
    return type(source)._from_packed(
        source.concat.detach(),
...
@NestedTensorAtenRegistry.implement(aten._to_copy.default, compile_safe=True)
def to_copy(func, args, kwargs):
    ...
    return source.packed_like(func(source.concat, **kwargs))
...
def _creation_like_handler(func, args, kwargs):
    ...
    source = args[0]
    return source.packed_like(func(source.concat, *args[1:], **kwargs))
...
def _ternary_handler(func, args, kwargs):
    ...
    if tuple(out_values.shape[1:]) == tuple(ref.concat.shape[1:]):
        return ref._packed_like_unchecked(out_values)
    if out_values.dim() == ref.concat.dim():
...
def _make_dim_reduction_handler(ragged_fill, default_dim, keepdim_kw):
    ...
    def _handler(func, args, kwargs):
        ...
        fill = ragged_fill
        if fill is ...:
            fill = _topk_fill_value(source.concat.dtype, largest=(func is aten.amax.default))
        return _dim_reduction_dispatch(
