...
def linear(input: NestedTensor, weight: Tensor, bias: Tensor | None = None) -> NestedTensor:
    ...
    return _apply(input, F.linear, weight, bias)
...
def _to_jagged(nt: NestedTensor) -> Tensor:
    ...
    """
    storage = nt._storage
    elem_ndim = storage[0].dim()
    if elem_ndim == 3:
        # (heads, seq_i, head_dim) -> (seq_i, heads, head_dim) for jagged
        flat = [t.transpose(0, 1).contiguous() for t in storage]
    ...
    return torch.nested.nested_tensor(list(storage), layout=torch.jagged)


def _from_jagged(jagged_out: Tensor, query: NestedTensor) -> NestedTensor:
    """Convert torch.nested jagged SDPA output back to DanLing NestedTensor."""
    from .nested_tensor import NestedTensor
    ...
    return NestedTensor(list(jagged_out.unbind()), **query._state)
...
def _sdpa_via_jagged(
    ...
    **kwargs,
    ...
    """Run SDPA via torch.nested jagged layout (Flash Attention variable-length path)."""
...
def cosine_embedding_loss(
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> from danling.tensors.functional import concat_tensors
        >>> input1 = NestedTensor(torch.tensor([[0.1, 0.2]]), torch.tensor([[0.3, 0.4]]))
        >>> input2 = NestedTensor(torch.tensor([[0.3, 0.4]]), torch.tensor([[0.1, 0.2]]))
        >>> target = NestedTensor(torch.tensor([1.0]), torch.tensor([1.0]))
        >>> out = F.cosine_embedding_loss(input1, input2, target)
        >>> target_concat, input1_concat, input2_concat = concat_tensors(target, input1, input2)
        >>> ref = F.cosine_embedding_loss(input1_concat, input2_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    target_concat, input1_concat, input2_concat = concat_tensors(target, input1, input2)
...
def hinge_embedding_loss(
    ...
    Examples:
        ...
        >>> out = F.hinge_embedding_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.hinge_embedding_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def cross_entropy(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.cross_entropy(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.cross_entropy(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def binary_cross_entropy(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.binary_cross_entropy(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.binary_cross_entropy(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def binary_cross_entropy_with_logits(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.binary_cross_entropy_with_logits(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.binary_cross_entropy_with_logits(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def nll_loss(
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> from danling.tensors.functional import concat_tensors
        >>> input = NestedTensor(torch.tensor([[-0.5, -1.0]]), torch.tensor([[-1.0, -0.5]]))
        >>> target = NestedTensor(torch.tensor([0]), torch.tensor([1]))
        >>> out = F.nll_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.nll_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def gaussian_nll_loss(
    ...
    Examples:
        ...
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> var = torch.ones(4)
        >>> out = F.gaussian_nll_loss(input, target, var)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.gaussian_nll_loss(input_concat, target_concat, var)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def poisson_nll_loss(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.poisson_nll_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.poisson_nll_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def l1_loss(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.l1_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.l1_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def smooth_l1_loss(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.smooth_l1_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.smooth_l1_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def soft_margin_loss(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.soft_margin_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.soft_margin_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def multi_margin_loss(
    ...
    Examples:
        ...
        >>> from danling.tensors.functional import concat_tensors
        >>> input = NestedTensor(torch.tensor([[0.1, 0.2]]), torch.tensor([[0.3, 0.4]]))
        >>> target = NestedTensor(torch.tensor([1]), torch.tensor([0]))
        >>> out = F.multi_margin_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.multi_margin_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def multilabel_margin_loss(
    ...
    Examples:
        ...
        >>> from danling.tensors.functional import concat_tensors
        >>> input = NestedTensor(torch.tensor([[0.1, 0.2]]), torch.tensor([[0.3, 0.4]]))
        >>> target = NestedTensor(torch.tensor([[1, 0]]), torch.tensor([[0, 1]]))
        >>> out = F.multilabel_margin_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.multilabel_margin_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def multilabel_soft_margin_loss(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.multilabel_soft_margin_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.multilabel_soft_margin_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def triplet_margin_loss(
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> from danling.tensors.functional import concat_tensors
        >>> anchor = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> positive = NestedTensor(torch.tensor([0.15, 0.25]), torch.tensor([0.35, 0.45]))
        >>> negative = NestedTensor(torch.tensor([0.9, 0.8]), torch.tensor([0.7, 0.6]))
        >>> out = F.triplet_margin_loss(anchor, positive, negative)
        >>> anchor_concat, positive_concat, negative_concat = concat_tensors(anchor, positive, negative)
        >>> ref = F.triplet_margin_loss(anchor_concat, positive_concat, negative_concat)
        >>> torch.allclose(out, ref)
        True
    """
    from .nested_tensor import NestedTensor
...
def triplet_margin_with_distance_loss(
    ...
    Examples:
        ...
        >>> positive = NestedTensor(torch.tensor([0.15, 0.25]), torch.tensor([0.35, 0.45]))
        >>> negative = NestedTensor(torch.tensor([0.9, 0.8]), torch.tensor([0.7, 0.6]))
        >>> out = F.triplet_margin_with_distance_loss(anchor, positive, negative)
        >>> anchor_concat, positive_concat, negative_concat = concat_tensors(anchor, positive, negative)
        >>> ref = F.triplet_margin_with_distance_loss(anchor_concat, positive_concat, negative_concat)
        >>> torch.allclose(out, ref)
        True
    """
    from .nested_tensor import NestedTensor
...
def margin_ranking_loss(
    ...
    Examples:
        ...
        >>> from danling.tensors import NestedTensor
        >>> from danling.tensors.functional import concat_tensors
        >>> input1 = NestedTensor(torch.tensor([0.1]), torch.tensor([0.3]))
        >>> input2 = NestedTensor(torch.tensor([0.2]), torch.tensor([0.4]))
        >>> target = NestedTensor(torch.tensor([1.0]), torch.tensor([-1.0]))
        >>> out = F.margin_ranking_loss(input1, input2, target)
        >>> input1_concat, input2_concat, target_concat = concat_tensors(input1, input2, target)
        >>> ref = F.margin_ranking_loss(input1_concat, input2_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    from .nested_tensor import NestedTensor
...
def huber_loss(
    ...
    Examples:
        ...
        >>> input = NestedTensor(torch.tensor([0.1, 0.2]), torch.tensor([0.3, 0.4]))
        >>> target = NestedTensor(torch.tensor([0.0, 0.1]), torch.tensor([0.2, 0.3]))
        >>> out = F.huber_loss(input, target)
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.huber_loss(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
...
def kl_div(
    ...
    Examples:
        ...
        >>> input_concat, target_concat = concat_tensors(input, target)
        >>> ref = F.kl_div(input_concat, target_concat)
        >>> torch.allclose(out, ref)
        True
    """
    input_concat, target_concat = concat_tensors(input, target)
