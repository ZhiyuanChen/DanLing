...
def multi_head_attention_forward(
    ...
    Examples:
        ...
        >>> torch.allclose(out, ref_out) and torch.allclose(attn, ref_attn)
        True
    """
    from .nested_tensor import NestedTensor
    ...
    def _padding_mask(nt: NestedTensor):
        mask = ~nt.mask if not nt.mask_value else nt.mask
        return mask if nt.batch_first else mask.transpose(0, 1)

    if key_padding_mask is None:
        if key_nt is not None:
            key_padding_mask = _padding_mask(key_nt)
        elif value_nt is not None:
            key_padding_mask = _padding_mask(value_nt)
        else:
            key_padding_mask = _padding_mask(query)

    def _to_seq_first(t: Tensor, batch_first: bool) -> Tensor:
        if batch_first and t.dim() >= 2:
            return t.transpose(0, 1)
        return t

    q_t = _to_seq_first(query.tensor, query.batch_first)
    if key_nt is not None:
        k_t = _to_seq_first(key_nt.tensor, key_nt.batch_first)
    else:
        k_t = key
        if isinstance(k_t, Tensor) and k_t.dim() == 3 and query.batch_first:
            k_t = k_t.transpose(0, 1)
    if value_nt is not None:
        v_t = _to_seq_first(value_nt.tensor, value_nt.batch_first)
    else:
        v_t = value
        if isinstance(v_t, Tensor) and v_t.dim() == 3 and query.batch_first:
    ...
    attn_output = attn_output.transpose(0, 1) if query.batch_first else attn_output
