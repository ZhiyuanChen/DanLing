...
def embedding(input: NestedTensor, weight: Tensor, padding_idx: int | None = None, max_norm: float | None = None, norm_type: float = 2.0, scale_grad_by_freq: bool = False, sparse: bool = False) -> NestedTensor:
    from .nested_tensor import NestedTensor

    return NestedTensor(F.embedding(t, weight, padding_idx, max_norm, norm_type, scale_grad_by_freq, sparse) for t in input._storage)
