...
def _zero_padding_safe_vector_ord(value) -> bool:
...
def _compare(input, other, op, **kwargs) -> bool:
    ...
    from .aten_functions import _layout_match
    from .nested_tensor import NestedTensor

    if op is torch.equal:
        ref = input if isinstance(input, NestedTensor) else other if isinstance(other, NestedTensor) else None
        dense = (
            other
            if isinstance(input, NestedTensor) and not isinstance(other, NestedTensor)
            else input if isinstance(other, NestedTensor) and not isinstance(input, NestedTensor) else None
        )
...
# _unary_handler on _values, which is faster than per-element _map_storage_serial.
...
def index_copy(input: NestedTensor, dim: int, index, source):
    ...
    if isinstance(source, NestedTensor):
        ...
        if not isinstance(index, NestedTensor):
            # Plain Tensor sources keep the eager semantics of reusing the same source per element,
            # so only NestedTensor sources are delegated to the aten handler.
...
def masked_select(input: NestedTensor, mask):
    ...
    if isinstance(mask, NestedTensor):
        ...
        try:
            # Packed masked_select is only valid when the mask is exact-shape and
            # shares offsets with the source; broader broadcasted masks stay per-element.
...
def nonzero(input: NestedTensor, *, out=None, as_tuple: bool = False):
    ...
    if not as_tuple:
        try:
            ...
            # supported packed layouts. Flattened storage keeps the explicit fallback.
...
def _scatter_aten(input: NestedTensor, dim: int, index, src):
    from .nested_tensor import NestedTensor
...
def scatter(input: NestedTensor, dim: int, index, src):
    ...
    Examples:
        ...
        True
    """
    return _scatter_aten(input, dim, index, src)
...
def bmm(input, mat2, *, out=None):
    ...
    Examples:
        ...
        True
    """
    from .aten_functions import _structure_match
    from .nested_tensor import NestedTensor
    ...
    if isinstance(input, NestedTensor) and isinstance(mat2, NestedTensor):
        ...
        if _structure_match(input, mat2) and input._values.dim() > 2 and mat2._values.dim() > 2:
...
def matmul(input, other, *, out=None):
    ...
    # Eager aten.matmul decomposes to lower ops that do not preserve NestedTensor
    # semantics yet; only delegate during graph capture where aten dispatch handlers apply.
    if (isinstance(input, NestedTensor) or isinstance(other, NestedTensor)) and torch.compiler.is_compiling():
        return torch.ops.aten.matmul.default(input, other)
...
def mm(input, mat2, *, out=None):
    ...
    if isinstance(input, NestedTensor):
        if input._values.dim() == 2 and not isinstance(mat2, NestedTensor):
            return torch.ops.aten.mm.default(input, mat2)
