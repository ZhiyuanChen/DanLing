...
class MetricMeters(AverageMeters):
    ...
    def set(self, name: str, meter: MetricMeter | Callable) -> None:  # type: ignore[override] # pylint: disable=W0237
        if callable(meter):
            meter = self.build_metric_meter_from_callable(meter)
        if not isinstance(meter, MetricMeter):
            raise ValueError(f"Expected meter to be an instance of MetricMeter, but got {type(meter)}")
        super().set(name, meter)

    def build_metric_meter_from_callable(self, func: Callable) -> MetricMeter:
...
class MultiTaskMetricMeters(MultiTaskAverageMeters):
    ...
    def set(  # pylint: disable=W0237
        ...
        if isinstance(meter, Metrics):
            meter = MetricMeters(
                preprocess=meter.preprocess,
                ignore_index=meter.ignore_index,
                ignore_nan=meter.ignore_nan,
                **meter.metrics,
            )
        elif callable(meter):
            meter = MetricMeter(meter, preprocess=None)
        elif not isinstance(meter, (MetricMeter, MetricMeters)):
            raise ValueError(
                f"Expected {meter} to be an instance of MetricMeter or MetricMeters, but got {type(meter)}"
            )
        super().set(name, meter)
