...
class MetricState:
    ...
    def _require_confmat(self, task: str) -> Tensor:
        if self.confmat is None:
            raise ValueError(f"{task} confusion-matrix stats require state.confmat.")
        return self.confmat.to(torch.float)

    @cached_property
    def multiclass_stats(self) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        confmat = self._require_confmat("Multiclass")
    ...
    @cached_property
    def multilabel_stats(self) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        confmat = self._require_confmat("Multilabel")
        tn = confmat[:, 0, 0]
        fp = confmat[:, 0, 1]
        fn = confmat[:, 1, 0]
        tp = confmat[:, 1, 1]
