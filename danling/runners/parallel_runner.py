...
class ParallelRunner(TorchRunner):
    ...
    _pipeline_loss_normalizer_local: float = 0.0
    _pipeline_loss_mode: str | None = None
    ...
    def __post_init__(self):
        self._pipeline_loss_normalizer_local = 0.0
        self._pipeline_loss_mode = None
    ...
    def _pipeline_loss(self, pred: Any, target: Any) -> torch.Tensor:
        ...
        normalizer = int(nested.numel()) if nested is not None else None
        normalizer_value = float(max(int(normalizer), 1)) if normalizer is not None else 1.0
        if self._pipeline_loss_mode is not None:
            self._pipeline_loss_normalizer_local += normalizer_value
            if self._pipeline_loss_mode == "train":
                self._accumulation_normalizer_local += normalizer_value
            return loss * normalizer_value
        return loss
    ...
    def loss_normalizer(self, data: Any) -> int | None:
        if self.pipeline_schedule is not None:
            return None
        return super().loss_normalizer(data)
    ...
    def _pipeline_loss_value(self, losses: list[torch.Tensor]) -> torch.Tensor | None:
        ...
        if self._pipeline_loss_normalizer_local > 0:
            loss = loss / self._pipeline_loss_normalizer_local
        else:
            loss = loss / len(losses)
        return loss

    def _sync_pipeline_accumulation_normalizer(self) -> None:
        if self.pipeline_degree <= 1:
            return
        if self.pipeline_group is None or not (dist.is_available() and dist.is_initialized()):
            return

        value = self._pipeline_loss_normalizer_local if self.pipeline_has_last_stage else 0.0
        device = self.all_reduce_device()
        ...
        if not self.pipeline_has_last_stage:
            self._accumulation_normalizer_local += float(payload.item())
    ...
    def train_step(self, data) -> Tuple[Any, torch.Tensor | None]:
        ...
        with self.train_context():
            self._pipeline_loss_normalizer_local = 0.0
            self._pipeline_loss_mode = "train"
            ...
            pred = None
            self._sync_pipeline_accumulation_normalizer()
            self.step()
        return pred, loss
