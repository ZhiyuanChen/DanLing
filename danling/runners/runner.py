    ...
    def read_config(
        ...
        **kwargs,
        ...
        if isinstance(checkpoint, Mapping):
            return BaseRunner.read_config(checkpoint, *args, **kwargs)
        if not isinstance(checkpoint, (bytes, str, os.PathLike)):
            return BaseRunner.read_config(checkpoint, *args, **kwargs)

        try:
            return TorchRunner.read_config(checkpoint, *args, **kwargs)
        except ValueError as torch_error:
            try:
                return DeepSpeedRunner.read_config(checkpoint, *args, **kwargs)
            except ValueError:
                raise torch_error
