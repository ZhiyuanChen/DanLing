...
class MetaRunner(type):
    def __call__(cls, *args: Any, **kwargs: Any) -> Any:
        ...
        # `Runner.__new__` can return an already-constructed concrete runner
        # instance (for example `TorchRunner`) when called as `Runner(config)`.
        # In that case, the inner concrete construction has already run
        # `__post_init__`, so the outer `Runner(...)` call must not run it again.
...
def format_result(result, format_spec: str = ".4f") -> str:
