...
class ParallelTopology:
    ...
    def __init__(
        ...
        *,
        ...
        inferred_axes = [axis for axis, degree in axis_degrees.items() if degree == -1]
        if len(inferred_axes) > 1:
            raise ValueError(f"invalid {label}: at most one axis degree may be -1 for auto-fill, got {inferred_axes}")
        ...
        known_topology_size = prod(degree for degree in axis_degrees.values() if degree != -1)
        if inferred_axes:
            inferred_axis = inferred_axes[0]
            if world_size % known_topology_size != 0:
                axis_product = " * ".join(f"{axis}({degree})" for axis, degree in axis_degrees.items())
                raise ValueError(
                    f"invalid {label}: WORLD_SIZE({world_size}) must be divisible by "
                    f"known axis product {axis_product} = {known_topology_size}"
                )
            axis_degrees[inferred_axis] = world_size // known_topology_size
