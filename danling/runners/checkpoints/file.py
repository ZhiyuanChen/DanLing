...
class FileCheckpointManager(CheckpointManager):
    ...
    def _persist_checkpoint_payload(self, task: CheckpointTask) -> None:
        ...
        published_aliases = [os.path.basename(latest_path)]
        success_target = os.path.basename(latest_path)

        if task.history_name is not None:
            history_path = os.path.join(checkpoint_dir, task.history_name)
            if self._update_checkpoint_alias(latest_path, history_path):
                published_aliases.append(task.history_name)
                success_target = task.history_name
                ...
                for stale_history in to_delete:
                    ...
                    self.enqueue_purge_path(stale_path)

        if task.should_update_best:
            best_path = os.path.join(checkpoint_dir, "best.pth")
            if self._update_checkpoint_alias(latest_path, best_path):
                published_aliases.append("best.pth")
    ...
    def _update_checkpoint_alias(self, source_path: str, alias_path: str) -> bool:
