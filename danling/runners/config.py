...
class RunnerConfig(chanfig.Config):  # pylint: disable=too-many-instance-attributes
    ...
    def __post_init__(self):
        compile_cfg = self.get("compile", {})
        self.compile = CompileConfig(compile_cfg if isinstance(compile_cfg, Mapping) else {})

        fsdp_cfg = self.get("fsdp", {})
        self.fsdp = FsdpConfig(fsdp_cfg if isinstance(fsdp_cfg, Mapping) else {})

        tppp_cfg = self.get("tppp", {})
        self.tppp = TpppConfig(tppp_cfg if isinstance(tppp_cfg, Mapping) else {})

        dtpp_cfg = self.get("dtpp", {})
        self.dtpp = DtppConfig(dtpp_cfg if isinstance(dtpp_cfg, Mapping) else {})

        checkpoint_cfg = self.get("checkpoint", {})
        self.checkpoint = CheckpointConfig(checkpoint_cfg if isinstance(checkpoint_cfg, Mapping) else {})
